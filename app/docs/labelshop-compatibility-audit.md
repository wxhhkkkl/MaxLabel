...
原版签赋LabelShop 是**多版本商业产品**：标准版 / 专业版 / 企业版按授权分层，激活方式有「用户登录 / 硬件锁（加密狗）/ 密钥注册」三种，另有「专业版演示模式」（体验专业版功能，打印时在标签上随机输出一行提示信息）。MaxLabel 是**单一版本产品**，本仓库不实现版本分层，理由与等价替代如下：

原版出处：帮助 `install_reg.html`（激活软件许可：用户登录 / 硬件锁 / 演示模式）与 `install_main.html`（三种激活方式）。

| 原版能力 | 复刻版处置 | 理由 |
| --- | --- | --- |
| 硬件锁（加密狗）激活 | 不实现；`window.maxlabel.license` 只暴露 `status` / `activate` / `check` 三个通道 | 需要厂商 SDK 与实体加密狗，无法在无硬件环境下产生用户可见行为；密钥激活已覆盖「安装后激活才能用全部功能」这一用户可见结果 |
| 专业版演示模式 | 不实现；打印链路（`src/shared/print/*`、`src/renderer/src/features/printing/*`）不存在任何演示水印注入点 | 演示模式的唯一可见效果是「打印时随机插入提示行」，在单版本产品里无对应语义，插入水印反而会破坏打印正确性 |
| 标准版 / 专业版 / 企业版分层 | 不实现；标题栏只保留激活状态一段（`src/shared/appTitle.ts` 的 `composeWindowTitle`，方括号内不含版本名） | 单一授权无版本维度；`账户(A) → 演示和体验` 入口保留并改为打开「版本与激活」说明（`GetStartedDialog` 的 `version` 主题），保留原版菜单肌肉记忆 |
| 「缩减量」「手动设置 128 码字符集」等企业版专属项 | 属性页不提供入口 | 登记在 `parity/matrix.md` 的对应条目证据列，属同一版本边界 |

该策略由 `app/scripts/license-single-version.test.ts`（`npm run test:license`）锁死：策略文本、授权 API 面、无硬件锁/演示模式入口、打印管线无水印注入、标题栏无版本分层，任一被改动即回归失败。

## 有意保留的实现差异

- LabelShop 的 MDI“新建窗口/层叠/平铺/排列图标”在 Electron 中由文档标签页替代。这些窗口排列命令保留为不可用菜单项，用来维持菜单肌肉记忆，但不复制已经淘汰的子窗口管理。
- MaxLabel 对象复制粘贴使用受校验的 JSON 剪贴板格式，支持多选对象、组合对象、跨模板和跨窗口粘贴；LabelShop 私有二进制对象格式仍无法在没有格式规范的情况下可靠互通。
