...
;(async () => {
  ...
  try {
    ...
    results['substring editor supports shared variable and reorder'] = substringState.previews.length === 2 && substringState.previews[0].includes('批次') && substringState.previews[1].includes('号') && substringState.shared && (substringState.placeholder || '').startsWith('如 BatchNo') && substringState.up
