...
;(async () => {
  ...
  try {
    ...
    results['database print dialog has start record and defaults'] = dialogText.includes('打印数量') && dialogText.includes('单签拷贝') && dialogText.includes('启始记录') && await evaluate('document.querySelector("[data-testid=print-dialog-start-record]")?.value === "1" && document.querySelector("[data-testid=print-dialog-count]")?.value === "8" && document.querySelector("[data-testid=print-dialog-copies]")?.value === "1"')
    await click('[data-testid="print-dialog-advanced"]'); await sleep(120)
    await click('[data-testid="print-advanced-tab-database"]'); await sleep(80)
    const databaseDialogText = await evaluate('document.querySelector("[data-testid=print-advanced-dialog]")?.innerText || ""')
    results['database print advanced labels match help'] = databaseDialogText.includes('打印时自动设置数据库记录数量') && databaseDialogText.includes('拷贝数量从数据库字段引入') && databaseDialogText.includes('允许打印时输入第一个标签的拷贝数量')
