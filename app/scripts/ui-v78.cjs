...
;(async () => {
  ...
  try {
    ...
    const confirmProps = async () => { await evaluate('(() => { const d=document.querySelector("[data-testid=object-props-dialog]"); const b=d&&[...d.querySelectorAll("button")].at(-1); b?.click(); return !!b })()'); await sleep(240) }
    ...
    results['text font page uses LabelShop font names and field wording'] = await openProps('text') && await click('[data-testid=object-props-tab-font]') && await evaluate('(() => { const d=document.querySelector("[data-testid=object-props-dialog]"); const options=[...d.querySelectorAll("select")].flatMap((s)=>[...s.options].map((o)=>o.value)); return d.innerText.includes("字体宽度缩放倍数") && d.innerText.includes("字间距") && ["Symbol","楷体","仿宋"].every((name)=>options.includes(name)) })()')
    await closeProps()

    await click('[data-tool=barcode]'); await clickCanvas(620, 170); await sleep(300)
    results['barcode human-readable fields use the help wording'] = await openProps('barcode') && await click('[data-testid=object-props-tab-datasource]') && await evaluate('(() => { const d=document.querySelector("[data-testid=object-props-dialog]"); return d.innerText.includes("供人识读的字符：位置") && d.innerText.includes("供人识读的字符：垂直偏移") && d.innerText.includes("供人识读的字符：对齐方式") })()')
    await closeProps()
