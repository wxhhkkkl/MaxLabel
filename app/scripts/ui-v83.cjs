...
;(async () => {
  ...
  try {
    ...
    results['C-79 已登录时文件菜单分享入口可用'] = await evaluate(`document.querySelector('[data-menu-item="分享(I)..."]')?.getAttribute('data-menu-disabled') === 'false'`)
    if (!await click('[data-menu-item="分享(I)..."]')) throw new Error('share menu item is disabled or missing: ' + await evaluate(`document.querySelector('[data-menu-item="分享(I)..."]')?.getAttribute('data-menu-disabled')`))
