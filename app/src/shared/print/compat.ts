...
export interface CompatEntry {
  brand: string
  model: string
  /** 本软件已经实现的指令集（按推荐优先级）。厂商的其他协议写入 note。 */
  engines: Array<'tspl' | 'zpl' | 'cpcl'>
  /** 常见端口 */
...
export const COMPAT_MATRIX: CompatEntry[] = [
  { brand: '佳博 Gprinter', model: 'GP-M32 / GP-1424D 等桌面机', engines: ['tspl', 'cpcl'], ports: ['driver', 'usb', 'com', 'tcp'], dpi: [203], note: '厂商还支持 ESC/ZPL 的部分型号；本软件当前生成 TSPL/CPCL' },
  { brand: '佳博 Gprinter', model: 'GP-M323', engines: ['tspl', 'cpcl'], ports: ['driver', 'usb', 'com', 'tcp'], dpi: [203], note: '官方页面列明 TSPL / CPCL / ESC-POS；本软件当前不生成 ESC-POS' },
  { brand: '汉印 HPRT', model: 'N41 / N31 桌面机', engines: ['tspl'], ports: ['driver', 'usb', 'com'], dpi: [203], note: '主流走 TSPL；ESC-POS 由部分型号支持但本软件当前不生成' },
  { brand: '汉印 HPRT', model: '便携/工业机型', engines: ['cpcl', 'tspl'], ports: ['driver', 'usb', 'bluetooth'], dpi: [203], note: 'CPCL 多在工业级，ZPL 仅部分跨界型号支持' },
  { brand: '芯烨 Xprinter', model: 'XP-D 系列桌面机', engines: ['tspl'], ports: ['driver', 'usb', 'com', 'tcp'], dpi: [203, 300], note: '兼容 TSPL 为主；ESC-POS 仅部分型号支持且不属于本软件当前实现' },
...
export function buildCompatChecklist(printer: PrinterConfig): string[] {
  ...
  if (!capabilities.supportsEllipse) items.push('椭圆对象：原生指令模式不保证输出，必要时切换驱动打印或预渲染位图')
  if (!capabilities.supportsRfid) items.push('RFID：当前指令集不提供标准写入能力，必须使用驱动/厂商 SDK 验证')
