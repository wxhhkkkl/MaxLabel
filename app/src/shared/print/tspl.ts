...
import type { BarcodeObj, DataCtx, LabelDoc, PrinterConfig, RfidObj, TextObj } from '../model'
...
export function buildTSPL(doc: LabelDoc, printer: PrinterConfig, ctx: DataCtx, warnings: string[]): string {
  const d = printer.dpi
  const L: string[] = []
  L.push(`SIZE ${fmt(doc.widthMm)} mm,${fmt(doc.heightMm)} mm`)
  ...
  if (printer.backfeedMm > 0) L.push(`SETTEAR ${fmt(printer.backfeedMm)} mm`)
  L.push('CLS')
  for (const obj of doc.objects) {
    if (obj.visible === false) continue
    if (obj.type === 'text') {
      const t = tsplText(obj, d, ctx, warnings)
      if (t) L.push(t)
    } else if (obj.type === 'barcode') {
      const b = tsplBarcode(obj, d, ctx, warnings)
      if (b) L.push(b)
    } else if (obj.type === 'rfid') {
      const r = tsplRfid(obj, d, ctx, warnings)
      if (r) L.push(r)
      ...
      warnings.push('TSPL 无椭圆指令，已跳过（驱动打印模式可输出）')
    } else if (obj.type === 'image') {
      const mono = ctx.images?.[obj.id]
      if (mono) {
        const x = mm2dot(obj.x, d)
        const y = mm2dot(obj.y, d)
        L.push(`PUTBMP ${x},${y},${q('img')}`)
        L.push(`@@BIN:${ctx.labelIndex}:${obj.id}@@`)
      } else {
        warnings.push('图片对象在指令打印中需下载位图（P1.2 实现），已跳过')
      }
    }
  }
  // PRINT m,n：m=打印张数（此处为单签拷贝），n=暂停间隔（0 不暂停）
  L.push(`PRINT ${Math.max(1, ctx.copy)},0`)
  return L.join('\n')
}

function tsplText(obj: TextObj, d: number, ctx: DataCtx, warnings: string[]): string {
  ...
  if (obj.arc) {
    ...
    if (mono) {
      const x = mm2dot(obj.x, d)
      const y = mm2dot(obj.y, d)
      return `PUTBMP ${x},${y},${q('arc')}\n@@BIN:${ctx.labelIndex}:${obj.id}@@`
    }
    warnings.push('弧形文字在 TSPL 需下载位图，已跳过')
    return ''
  }
  ...
  if (/[^\x00-\x7F]/.test(text)) {
    ...
    if (mono) {
      const x = mm2dot(obj.x, d)
      const y = mm2dot(obj.y, d)
      return `PUTBMP ${x},${y},${q('txt')}\n@@BIN:${ctx.labelIndex}:${obj.id}@@`
    }
    warnings.push('含中文的文本在 TSPL 需下载中文字体（P1.2），当前按内置字体输出')
  }
  ...
  return `TEXT ${x},${y},${q(fontName)},${rot(obj.rotation) / 90},${xmult},${ymult},0,${q(text)}`
}

/** RFID：写入标签（需 RFID 打印头）。数据按数据源解析，支持 EPC/USER/TID 区 */
function tsplRfid(obj: RfidObj, d: number, ctx: DataCtx, warnings: string[]): string {
  const text = resolveObjectText(obj, ctx)
  if (!text) return ''
...
function tsplBarcode(obj: BarcodeObj, d: number, ctx: DataCtx, warnings: string[]): string {
  const text = resolveObjectText(obj, ctx)
  if (!text) return ''
  const x = mm2dot(obj.x, d)
  const y = mm2dot(obj.y, d)
  const h = mm2dot(obj.h, d)
  const r = rot(obj.rotation) / 90
  if (obj.symbology === 'qrcode') {
    warnings.push('TSPL 二维码语法因固件而异，请在真机验证')
    return `QRCODE ${x},${y},M,4,${q(text)}`
  }
  if (obj.symbology === 'pdf417') {
    warnings.push('TSPL PDF417 语法请在真机验证')
    return `PDF417 ${x},${y},${mm2dot(obj.w, d)},2,3,0,${h},${q(text)}`
  }
  if (obj.symbology === 'datamatrix') {
    warnings.push('TSPL DataMatrix 语法请在真机验证')
    return `DATAMATRIX ${x},${y},${Math.max(8, h)},${q(text)}`
  }
  const type = TSPL_SYM[obj.symbology] ?? '128'
  return `BARCODE ${x},${y},${q(type)},${h},${obj.showText ? 1 : 0},${r},1,1,0,${q(text)}`
}

/** 表格：外框 BOX + 内部网格 LINE（TSPL 原生支持） */
function tsplTable(obj: import('../model').TableObj, d: number, L: string[]): void {
  const x1 = mm2dot(obj.x, d)
  const y1 = mm2dot(obj.y, d)
  const x2 = mm2dot(obj.x + obj.w, d)
  const y2 = mm2dot(obj.y + obj.h, d)
