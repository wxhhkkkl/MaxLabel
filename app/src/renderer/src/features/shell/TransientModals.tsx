      ...
      <div style={{ background: '#fff', padding: 20, borderRadius: 12, width: 380, maxWidth: '92vw', boxShadow: '0 8px 40px rgba(0,0,0,0.3)' }} onClick={(e) => e.stopPropagation()}>
        ...
        {labels.map((label) => (
          <div key={label} style={{ marginBottom: 10 }}>
            ...
            <input value={vals[label] ?? ''} onChange={(e) => setVals((current) => ({ ...current, [label]: e.target.value }))} style={{ width: '100%', padding: '8px 10px', border: '1px solid #D5D4CD', borderRadius: 6, fontSize: 13, boxSizing: 'border-box' }} autoFocus={labels.length === 1} />
          </div>
        ))}
