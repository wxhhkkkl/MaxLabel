...
## DIFF-63（未收口，待真机点按）序列号面板的「重置初始值 / 立即重置」
...
**在真机点按确认前不实现**（避免按猜测造功能）。复刻版现状：`序列起始值` 覆盖取值语义，
`resetEachRecord`（按标签变化时每条记录开始复位到初始值）覆盖归位；无独立「立即重置」入口。
待真机用真实鼠标点选数据源下拉到「序列号」，再读子面板控件与点按「立即重置」的表现。
