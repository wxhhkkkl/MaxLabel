...
- **状态**：未实现（三件效果**功能都在**，但控件形态与真机不同：真机是组框内的三个**复选框**，复刻版是一排按钮 + 一个孤立复选框）。
...
- **处置**：**待做**（归入 TOP 待办的「字体页对齐」那一轮）——改成组框 + 三个复选框、顺序与加速键 `(&S)/(&U)/(&W)` 照 dump。
- **断言**：暂无断言（未实现；改完后应补「组框 legend 整数组全等 + 三个复选框文案含加速键」）。

## DIFF-90（round-139 补登 —— **本轮之前三处台账都查不到**；原版有 / 复刻版文案缺字）字体页「字体宽度方向缩放倍数(&H):」

- **状态**：文案不一致（**缺「方向」二字与加速键 `(&H)`**）。
