...
## DIFF-80（round-132 新登记，**未实现**）`账户(A)` 菜单缺真机有的 `服务器...` 一项
...
**复刻版现状**：`labelShopMenus.ts` 的 `账户(A)` 只有 6 项（`登录...` / `注销...` / `账号和授权管理...` / `试用管理...` / `演示和体验...`），
无 `服务器...`；全库亦无「连接密钥 / 新增服务器连接 / 授权服务器设置」等文案 → **不是等价替代，是真缺口**。

**处置**：**不实现**（本轮无该入口的行为证据：点开后是列表对话框还是属性表、`确定` 后落到哪里、与系统选项里的单一
服务器地址字段是如何共存的，都还没取证）。按要求登记为待实现缺口，取证手法与「优先级 1」的 DIFF-63 同（真机菜单 + 树 dump）。
**断言**：暂无（未实现，不写"存在性"假断言）。
