...
## DIFF-85（round-136 登记并处置；**状态：✅ 已结案 —— 三条均不改产品代码**。证据 = 真机原图三张：`probe-r112-sysset.png`（标题 `系统设置`）、`probe-15-cloudbox-port.png`（`类型(I):`）、`verifier-r44-hole-circle-20b.png`（底排 `确定/取消/帮助`）；验收方施工清单 C1/C3 系读图误判、C2 代码本已为 `类型(I):`）标题/文案类
...
## DIFF-87（round-139 补登 —— **本轮之前台账漏登**；原版有 / 复刻版缺）数据源页「子串列表」

- **状态**：未实现（复刻版**无**该项；此前只在 `parity/matrix.md` 的 5 行里被提到，`diffs.md`/`backlog.md` 都查不到 → 验收方 round-233 指出会漏统计）。
- **真机**（`parity/reference/labelshop/probe-60-barcode-props-tree.txt` 第 20 行，数据源页）：
  `[V] class=Static text='子串列表' xy=(928,455) wh=(138x24)`，其正下方是
  `[V] class=ListBox xy=(928,488) wh=(245x414)` —— 即数据源页左侧一栏**子串条目列表**（可容纳多条子串规则的列表框）。
- **复刻版**：无。全库 `子串列表` 零命中；模型里只有一个 `substr?: {start,length}` 单条规则（`ObjectPropsDialog.tsx:1597/1610` 两个自造名输入框）。
- **处置**：**待做**（归入「数据源页」那一轮，与 DIFF-71/66 同一施工口径）——真机的模型是**多条子串规则 + 列表**，
  复刻版是单条；按验收方口径这属**模型级分歧**，动手前需先做「按真机改 vs 登记为复刻版扩展」的决策（见 `parity/backlog.md` 顶部 TOP 待办）。
- **断言**：暂无断言（未实现；真机截图/控件树已备，见上）。

## DIFF-88（round-139 补登 —— **本轮之前台账漏登**；原版有 / 复刻版缺）数据源页「子串选项」组框

- **状态**：结构未实现（复刻版有同名功能的字段，但**没有真机的组框与命名**）。
- **真机**（`parity/reference/labelshop/probe-60-barcode-props-tree.txt` 第 21 行，数据源页）：
  `[V] class=Button text='子串选项' xy=(1192,455) wh=(440x489)` —— 一个 440×489 的**分组框**，
  其内是子串规则的各项参数（起始/长度等）。
- **复刻版**：`ObjectPropsDialog.tsx:1597-1620` 两个**平铺 FormField**，自造名 `子串起始（0 起）` / `子串长度（0=全部）`；
  无 `子串选项` 组框、无真机原文与加速键。
- **处置**：**待做**（同上，随数据源页那一轮）——补 `子串选项` 组框并把字段文案按真机原文对齐；字段是否改名需先定 DIFF-87 的模型决策。
- **断言**：暂无断言（未实现）。

## DIFF-89（round-139 补登 —— **本轮之前台账漏登**；原版有 / 复刻版形态不同）字体页「特殊效果」组框

- **状态**：✅ 已修（round-143）——`特殊效果` 组框 + 三个复选框（顺序 删除线 → 下划线 → 黑底白字，与 dump 一致），放在 `data-testid="font-group-effects"`。
- **真机**（`parity/reference/labelshop/probe-r201-textprops-font-tree.txt`）：
  `[V] class=Button text='特殊效果' xy=(931,722) wh=(696x144)`（组框），内含
  `删除线(&S)`(972,752) / `下划线(&U)`(972,788) / `黑底白字(&W)`(972,824) —— **三个 Button 型复选框，顺序为 删除线 → 下划线 → 黑底白字**。
- **复刻版**：`ObjectPropsDialog.tsx:455-474` —— `下划线`/`删除线` 是两个**普通按钮**（顺序与真机相反）、`黑底白字` 是**复选框但不在任何组框内**；
  无 `特殊效果` 组框（全库 `特殊效果` 零命中）。
- **处置**：改成组框 + 三个复选框、顺序与加速键照 dump（`ObjectPropsDialog.tsx` 字体页）。顺带把 `颜色` 也按真机改成 `颜色(&C)...` **按钮**
  （真机 `Button xy=(1470,806) wh=(132x45)`），把 `背景` / `打印机内建字体` 两项真机没有的字段移入标注过的扩展区。
- **断言**：`app/scripts/ui-v144.cjs` —— 「`特殊效果` 组里恰好 3 个复选框」「三个复选框文案逐字 = 删除线(S)/下划线(U)/黑底白字(W)」「三个复选框都接上加速键 s/u/w」
  「`颜色(C)...` 是按钮且带色块」「勾选删除线/下划线后预览即时跟随」。
  迁移 `app/scripts/ui-v95.cjs`（A-127 改读复选框勾选态、A-129 改读按钮背后隐藏取色输入的值，**仍是值级判定，强度不降**）。

## DIFF-90（round-139 补登 —— **本轮之前三处台账都查不到**；原版有 / 复刻版文案缺字）字体页「字体宽度方向缩放倍数(&H):」

- **状态**：✅ 已修（round-143）——文案改为真机原文 `字体宽度方向缩放倍数(&H):`（屏幕口径 `字体宽度方向缩放倍数(H):`，走 `displayMfcCaption`），
  `字间距` 同步改为 `字间距(&J):` + 独立 `毫米`。
- **真机**：`parity/reference/labelshop/probe-r201-textprops-font-tree.txt` 第 18 行
  `[V] class=Static text='字体宽度方向缩放倍数(&H):' xy=(1181,758) wh=(267x24)`，右侧是 Edit + spinner（`probe-45-barcode-props-p4.txt` 里该 Edit 的 `label` 同样带「方向」与 `(&H)`，值 `1.00`）。
  多份 dump 互相印证：`probe-38-text-props-tab2.txt:18` / `probe-60-barcode-props-tree.txt:99` / `probe-r201-textprops-text-tree.txt:46`。
- **复刻版**：`ObjectPropsDialog.tsx:434` `<FormField label="字体宽度缩放倍数" hint="默认 1.00">` —— 缺「方向」、缺 `(&H)`（同页 `字间距` 亦缺 `(&J):`）。
- **处置**：文案改真机原文（已做）；`ui-v78.cjs:95` 与 `ui-v125.cjs:163` 按**真机原文**迁移（**这是加严**：新断言还要求
  `字体宽度方向缩放倍数(H):` / `字间距(J):` 逐字 + 页内不含字面量 `&`，不是放宽）。
- **断言**：`app/scripts/ui-v144.cjs`「`字体宽度方向缩放倍数(H):` 是数字框且 min=0.1 max=10，接了加速键 h」+
  「`字间距(J):` 是数字框、右侧有独立 `毫米` 文本，接了加速键 j」；`ui-v78` / `ui-v125` 同步迁移并被 `ui-v144` 覆盖。
...
## DIFF-92（round-142 新登记并**已修**；原版有 / 复刻版形态与文案不同）「对象属性 → 文本」页按真机控件树对齐

- **状态**：✅ 已修（round-142）。四要素依据：`parity/reference/labelshop/probe-r201-textprops-text-tree.txt`（真机「文字属性 → 文本」页控件树）+
  `parity/P1-对象属性对照表.md` + `parity/验收方逐项比对-施工清单.md` J 节。
- **真机（dump 原文，逐条）**：
  1. 「类型」是**三个 Button 单选**：`单行(&S)`(1025,545) / `多行(&M)`(1223,545) / `圆形(&C)`(1421,545)，均 121×30、同 y、x 等距 → 一组单选，**不是下拉**；
  2. 「类型」与「属性」是两个**分组框** Button：`类型`(928,503) 704×102、`属性`(928,632) 704×378；
  3. `水平对齐(&A):`(950,671) Static + ComboBox(1099,665)；
  4. `行宽度(&W):`(1297,671) Static **DISABLED** + Edit(1418,665) **DISABLED** + `毫米`(1544,671) **DISABLED**（单行态）；
  5. `字符模板(&T):`(950,812) **Button(复选框)** 179×30 + Edit(1135,809) **DISABLED**；
  6. `文字停靠(&P)`(950,719) / `角度(&E):`(1297,719) / `行距(&L):`(950,767) / `弧度(&R):`(1297,767) / `字符剪裁(&C):`(948,1040) 行首均为 `[ ]`
     ＝**单行态不显示**（按模式出现，不是删掉）。
- **复刻版（改前）**：`ObjectPropsDialog.tsx` 的文本页是 `通用/文字/字体/数据` 四页签时代的旧结构 ——
  「类型」是**下拉**（`data-testid=text-type`）、**没有** `水平对齐`、`行宽度` 写作 `行宽度（毫米）`（单位写进标签）、
  `字符模板` 是普通输入框、`文字停靠` 常显；且多出 `大小写转换`/`子串起始`/`子串长度`/`截短`/`字符数限制` 等真机没有的字段而**未标注**。
- **处置**：
  ① 「类型」改三个 `radio`（同 `name`、默认选中 `单行`），放进 `text-group-type` 分组框，并逐个接上 `accessKey`（s/m/c）；
  ② 新增 `text-group-props` 分组框（真机「属性」），内含 `水平对齐(&A):`（沿用原 `align` 绑定，选项 left/center/right/justify 不变）、
     `行宽度(&W):`（真机文案 + 单位拆成独立 `毫米` + **单行态置灰**）、`字符模板(&T):`（**复选框 + 输入框**，未勾选⇒`charTemplate` 置 `undefined`＝不启用模板，
     勾选后才可编辑——绑的仍是正式模型字段，不是假实现）；
  ③ 按模式显示：`文字停靠(&P)`/`行距(&L):` 只在**多行**、`弧度(&R):`/`角度(&E):`/半径/回绕方向/文字方向 只在**圆形**；单行态一律不显示（与 dump 的 `[ ]` 一致）；
  ④ 真机没有的字段集中进 `data-testid="text-extensions"` 的**虚线扩展区**并写明「复刻版扩展（原版「文本」页中无以下字段）」——
     按验收方口径「不静默删功能，但必须明确标注」处理；
  ⑤ 顺手接上加速键 `a/w/t/p/l/r/e`（P3），本文件 `accessKey` 14 → 22。
- **断言**：`app/scripts/ui-v143.cjs` **21/21 PASS**（分组框整数组全等 / 三个 radio / 文案与顺序逐字 / 默认单行 / 加速键接线 /
  单行态 `行宽度` DISABLED / 单行态不显示 `文字停靠` / 多行态出现 `文字停靠` 且行宽度可用 / 圆形出现半径 / 字符模板勾选后才可编辑 / 扩展区标注）；
  迁移 `app/scripts/ui-v89.cjs`（4 → **6/6**，改成点 radio 并新增两条「点选后类型切换」断言）与 `app/scripts/ui-v56.cjs`（11 → **12/12**，
  拆成「单行态不显示文字停靠」+「多行态出现」两条）——**断言强度只增不减**。
  命令：`MAXLABEL_UI_SCRIPT=ui-v143.cjs npm run test:ui`。
- **待取证（本轮未做，不猜）**：真机 `文字停靠(&P)`/`角度(&E):`/`弧度(&R):` 在**多行/圆形**各态下的可见组合尚未单独 dump（本轮的"按模式显示"是
  从"单行态不可见"推出来的，已在上面写明；若后续 dump 出更细的组合，按新证据调整）。

## DIFF-93（round-143 新登记并**已修**；原版有 / 复刻版形态不同）字体页「示例」组框与 `大小(&P):`

- **状态**：✅ 已修（round-143）。
- **真机**（`parity/reference/labelshop/probe-r201-textprops-font-tree.txt`）：
  `[V] class=Button text='示例' xy=(931,875) wh=(696x156)`（**分组框**），内含
  `[V] class=Static text='显示示例' xy=(953,902)`，组下方 `[V] class=Static text='这是TRUETYPE字体，显示与打印完全相同!' xy=(928,1040)`
  —— 注意是**半角 `!`、无空格**；同页 `[V] class=Static text='大小(&P):' xy=(1506,458)` 旁是 **ComboBox**（不是数字框）。
- **复刻版（改前）**：`ObjectPropsDialog.tsx` 字体页的「示例」只是普通 `div`（不是组框）、说明文字写作
  `这是 TRUETYPE 字体，显示与打印完全相同！`（**多两个空格 + 全角 `！`**）；`大小` 早在 round-13x 已改下拉（这条本来就对）。
- **处置**：① `示例` 改成 `data-testid="font-group-sample"` 分组框；② 说明文字改为 dump 逐字（半角 `!`、无空格），加 `data-testid="text-font-truetype-note"`；
  ③ 组内加 `显示示例` 小标题（`data-testid="text-font-sample-label"`）；④ 真机没有的 `打印机内建字体` 与 `背景` 移入 `font-extensions` 虚线区并标注「复刻版扩展」。
- **断言**：`app/scripts/ui-v144.cjs` —— 「`示例` 组含 `显示示例` 且预览块存在」「`这是TRUETYPE字体，显示与打印完全相同!` 逐字一致（半角 !、无空格）」
  「字体页分组框整数组全等 = [特殊效果, 示例, 复刻版扩展…]」。命令：`MAXLABEL_UI_SCRIPT=ui-v144.cjs npm run test:ui`。
