...
## DIFF-50（观察项，未收口）对象属性「常规」页的「水平/垂直」相对标签边对齐下拉：真机是灰的 → ⏳ 待定
...
**为什么先不改**：真机在本版本 + 本对象类型下不启用，不能排除它对别的对象类型（图片/表格/条码）启用；
贸然改成灰的会把帮助里明确记载的字段做没。**下一步**：拿条码/图片/表格对象各开一次属性页确认启用条件，再决定是否收口。
...
## DIFF-64（未收口）对象属性对话框的**页签名称与顺序**与真机不一致，且条码多出一个「码制专页」
...
⑤ **条码属性真机只有 4 个页签**，复刻版多出一个码制专页——真机的码制专属选项应在「条码」页内，需按真机合并/迁移。

要求：按上表改显示名与顺序（key 可保留英文）、把条码码制专页并入「条码」页（或按取证再确认）、
补「页签文字与顺序逐项相等」的 CDP 断言、更新 matrix 涉及页签的条目证据，并删掉 `propertyTabs.ts` 里已被实拍证伪的
「通用页永远置于首位」注释。

---

## DIFF-65（待核对）真机「工具」菜单没有 RFID 工具，复刻版有
...
要求：先取证原版 RFID 对象的创建入口（候选：`数据(D)`、其它菜单、或在某种打印机/模板下才出现），
按取证结果把复刻版入口对齐（保留则必须给出原版入口证据；原版确无则移除或标注等价替代），不要凭帮助页保留。
