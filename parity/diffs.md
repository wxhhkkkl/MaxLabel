...
## DIFF-63（未收口，待取得可显示重置组的模板/数据库状态）序列号面板的「重置初始值 / 立即重置」
...
## DIFF-65（未收口；round-6 已排除「数据(D)」候选）真机「工具」菜单没有 RFID 工具，复刻版有

**验收方 round-5 取证**（`verifier-tools-menu.png`）真机工具菜单顺序：
`选取(S) / 条码(B) / 文字(T) / 线条(L) / 斜线(L) / 矩形(R) / 图片(P) / 数据(D) / 表格(G)` + `放大 / 缩小 / 适应宽度 / 适应高度 / 适合窗口`。

即**没有 RFID 工具项**。复刻版工具栏与工具菜单里有 RFID 入口（matrix 的 RFID 条目依据是帮助页）。
**round-6 追加取证**：把工具菜单第 8 项 `数据(D)` 拖出的对象打开属性，`listctl` 报根窗口标题＝**「条码属性」**
...
1. 先在编辑器放一个**明显的大对象**（整张贴满的矩形）再打印，确认 PDF 非空；仍空则先修打印链路。
...
## DIFF-71（round-112 新登记）「系统选项」对话框与真机「系统设置」的**页签结构与页内容**不一致 —— 真机 4 页，复刻版 3 页且其中一个页在真机无对应物
...
| 底部按钮 | `保存` → **`确定`**（真机原文）；另两个 `取消` / `帮助` 中 `帮助` 复刻版仍未挂 |
...
## DIFF-72（round-113 新登记，**已修**）真机「条码属性 → 条码」页的字段原文与「条码页有没有颜色」
...
| `对齐方式(&A):` | `供人识读字符 · 对齐方式` |
| **条码页无颜色控件**；`颜色(&C):` 在**常规**页 | `条码颜色` 色块挂在**条码**页 |

**round-65 冲突定案**：真机「条码」页（Code 128）dump 只有 **8 个控件**、其中**没有任何颜色项**；同一窗口的「常规」页 dump 里才有 `颜色(&C):`（值=固定颜色）。
→ 先前 `PROBE-verifier-object-tabs.md` 记的"条码页页尾有颜色色块"**不成立**，按"条码页无颜色"处理。

**处置**：字段名逐字改真机原文（`ObjectPropsDialog.tsx`），`条码颜色` 移到**常规**页并按真机改名 `颜色(&C):`。
证据：`parity/reference/labelshop/PROBE-round113-barcode-page.md`；断言 `app/scripts/ui-v125.cjs`（19/19，含"条码页无颜色控件"与"常规页有 颜色(&C):"两条新增）、`app/scripts/ui-v77.cjs`（7/7，B-69b 加严）。
