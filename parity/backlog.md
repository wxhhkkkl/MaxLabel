## round-143 结算（TOP 待办 #3：对象属性「字体」页按真机 dump 对齐）

- [x] **字体页对齐**（TOP #3）：依据 `parity/reference/labelshop/probe-r201-textprops-font-tree.txt` + `parity/P1-对象属性对照表.md`。改动：
      ① 首行三字段恢复真机文案 `字体名称(&T):` / `字体样式(&Y):` / `大小(&P):`（**三者都是 ComboBox**，屏幕口径不带 `&`，走 `displayMfcCaption`）并接加速键 t/y/p；
      ② 新增 `特殊效果` 组框（`font-group-effects`）＝ 三个**复选框** `删除线(&S)` / `下划线(&U)` / `黑底白字(&W)`（顺序照 dump），接加速键 s/u/w；
      ③ `字体宽度方向缩放倍数(&H):`（DIFF-90，补「方向」二字）+ `字间距(&J):` ＋ 独立 `毫米`，接加速键 h/j；
      ④ `颜色` 由内联色块改成真机形态 `颜色(&C)...` **按钮**（Button 132×45），背后挂隐藏取色输入，按钮内带色块回显；
      ⑤ 新增 `示例` 组框（`font-group-sample`）＝ `显示示例` + 预览块 + `这是TRUETYPE字体，显示与打印完全相同!`（**半角 `!`、无空格**，逐字照 dump）；
      ⑥ 真机没有的 `打印机内建字体` / `背景` **不静默删**，集中进 `font-extensions` 虚线区并标注「复刻版扩展」。
      台账：`parity/diffs.md` **DIFF-89 / DIFF-90 转为「✅ 已修」**、新登记 **DIFF-93**（示例组框 + 大小下拉），四要素齐。
      断言：新增 `app/scripts/ui-v144.cjs` **20/20**（已注册进 `run-regression.ps1`）；
      迁移 `app/scripts/ui-v95.cjs`（A-127 改读复选框勾选态、A-129 改读按钮背后的取色输入 → **16/16**，仍是值级判定，强度不降）、
      `app/scripts/ui-v78.cjs`（**10/10**，文案改真机原文并加严：另要求 `特殊效果` + TRUETYPE 逐字 + 页内无字面量 `&`）、
      `app/scripts/ui-v125.cjs`（**19/19**，同上加严）。
      matrix `B-61` / `B-62` / `B-63` / `B-64` 四行证据已同步。

- [x] **TOP #4（打印对话框名称/位置 + 属性窗口标题）**：**round-140 的 `67c160f` 已完成**（不是本轮的新活）。
      本轮只补了一处确定性：`openPrintDialog()` 打开对话框时**再取一次** `listPrinters()`
      （`App.tsx` 新增 `refreshSystemPrinters`），避免启动时那次枚举尚未返回时短暂显示占位词。
      实测（`node tools/parity/Check-PrintDialogFields.cjs`，本机 CDP 实例）：名称 = `Gprinter GP-1324D (USB)`、位置 = `USB001` ✓ 真实设备名与端口。
      回归：`ui-v142` **6/6**、`ui-v123` **17/17**。

- [x] **TOP #1（补 4 项台账漏登）**：**round-139 已完成**（`parity/diffs.md` 的 DIFF-87/88/89/90 四要素齐，`audit-diffs` 不再报这四条）。

### 本轮新登记的缺口 / 待办

- [ ] **TOP #5 发 v1.0.21** 仍待办（未做）：先读 `tools/loop/logs/round-138-gates.md` 的实际 `N/N` 校正 `RELEASE-NOTES-v1.0.21.md` 里的数字再打包。
      ⚠️ **数字一定已过时**：本仓库 `ui-v*.cjs` 现为 **96 个**（round-142 新增 ui-v143、本轮新增 ui-v144），说明里的 `91/91` 与 `N/N` 都要重读门禁日志后改。
- [ ] **TOP #2 的判据「`Verify-ObjectProps` 一致条数 > 10/32」已达成** ✓：本轮实测 **18/32**（文本页 4 条 round-142 转绿 + 字体页 4 条本轮转绿）。
      剩余 GAP 分两类：**8 条要求页面上出现字面量 `(&X)`**（与 DIFF-83 矛盾 → **永远不可能变绿**，见下条）+ **数据源页 5 条**（未做，下一轮）。
      → 可达上限是 **24/32**，不是 32/32。
- [ ] **`tools/parity/Verify-ObjectProps.cjs` 的 8 条 `(&X)` 字面量断言**（round-142 已登记，本轮复核仍成立）：口径与 DIFF-83 冲突，
      建议改成真机**屏幕口径**（`字体名称(T):` 而非 `字体名称(&T):`）。工具归验收方维护，本轮**未改**。
- [ ] **`tools/parity/audit-accelerators.cjs` 不过滤注释**（round-142 已登记，本轮复核仍成立）：它全文扫引号/反引号串，把注释里的 dump 原文也算成"标题字面量"，
      缺口被系统性高估（本轮实测：字面量 98 / accessKey 35 / 估算缺口 67）。**趋势才是有效信号**：本轮本文件 accessKey 22 → 31、全仓 26 → **35**。
      建议工具侧加注释过滤；在此之前 TOP #3 判据里的「缺口 < 46」无法直接比对。
- [ ] **下一轮建议做 TOP 待办里的「数据源页」（G 类）**：`Verify-ObjectProps` 剩下的 5 条 GAP 全在数据源页
      （`子串列表` / `数据源(S):`＋`显示数据(D):` / `变量共享名称(N):` / `高级选项(A)...` / `示例`）。
      ⚠️ 按验收方口径**先做决策**（按真机改 or 登记为复刻版扩展）再动手，不要直接照 dump 重写。
- [ ] **字体页的一处待取证**（本轮未做，不猜）：dump 里 `示例` 组框的矩形是 y 875..1031，而 `这是TRUETYPE字体…` 在 y=1040（组框**下方**）；
      验收方的对照表把它算在组内。本轮按"组内"实现（视觉上也属于该组）。若后续有更细的 dump 再定。

---

## round-142 结算（TOP 待办 #2：对象属性「文本」页按真机 dump 对齐）

- [x] **文本页对齐**（TOP #2）：依据 `parity/reference/labelshop/probe-r201-textprops-text-tree.txt` +
      `parity/P1-对象属性对照表.md` + `parity/验收方逐项比对-施工清单.md` J 节。改动：
      ① 「类型」由**下拉**改成**三个单选**（`单行(&S)`/`多行(&M)`/`圆形(&C)`，同 name、默认单行）；
      ② 补 `水平对齐(&A):` 下拉；③ `行宽度(&W):` 改真机文案、单位拆成独立 `毫米`、**单行态置灰**；
      ④ `字符模板(&T):` 改**复选框+输入框**（未勾选 ⇒ `charTemplate` 置 undefined ＝ 不启用模板，绑的仍是正式模型字段，不是假实现）；
      ⑤ 补真机两个分组框 `类型` / `属性`；⑥ 按模式显示：`文字停靠`/`行距` 仅多行，`弧度`/`角度`/半径 仅圆形，
      单行态一律不显示（与 dump 行首 `[ ]` 一致）；⑦ 真机没有的字段**不静默删**，集中进 `text-extensions` 虚线区并标注「复刻版扩展」；
      ⑧ 顺手接加速键 a/w/t/p/l/r/e（本文件 accessKey 14 → 22、全仓 18 → 26）。
      台账：`parity/diffs.md` **DIFF-92**（状态/证据/处置/断言四要素齐）。
      断言：新增 `app/scripts/ui-v143.cjs` **21/21**（已注册进 `run-regression.ps1`）；
      迁移 `app/scripts/ui-v89.cjs`（4 → **6/6**，改成点 radio + 新增两条「点选后类型切换」断言）与
      `app/scripts/ui-v56.cjs`（11 → **12/12**，拆成「单行态不显示文字停靠」+「多行态出现」两条）—— **断言强度只增不减**。
      matrix `B-65` / `B-66` / `B-67` 三行证据已同步。

### 本轮新登记的缺口 / 待办

- [ ] **TOP #3 字体页对齐**（下一轮最该做的）：`大小` 改下拉、`特殊效果` 组框+三复选框、`示例` 整块、
      `字体宽度方向缩放倍数(&H):` / `字间距(&J):` 文案、底排 `帮助`；依据 `probe-r201-textprops-font-tree.txt`。
      预告：`ui-v78.cjs:89` / `ui-v125.cjs:163` 的 `字体宽度缩放倍数` 断言会按 DIFF-90 预期迁移（不是产品回归）。
- [ ] **`tools/parity/Verify-ObjectProps.cjs` 里有 8 条断言要求页面上出现字面量 `(&X)`**（如 `位置锁定(&L)` / `水平(&H)` / `字体名称(&T)`）——
      这与 DIFF-83 的结论**矛盾**（真机屏幕上不显示 `&`，复刻版渲染一律走 `displayMfcCaption`），所以这几条**永远不可能变绿**。
      建议把期望改成真机**屏幕口径**（去掉 `&`，如 `位置锁定(L)`）；在此之前「一致条数」的天花板是 24/32 而非 32/32。
      本轮文本页那 4 条已从 2/4 转成 4/4（radios / 水平对齐 / 行宽度 / 字符模板）。
- [ ] **`tools/parity/audit-accelerators.cjs` 把注释里的 dump 引用也算成「标题字面量」**：它直接全文正则扫引号/反引号字符串、
      **不过滤注释行**，而本仓库注释大量用反引号引用真机 dump 原文（如 `单行(&S)`）→ 缺口被系统性高估。
      实测 `ObjectPropsDialog.tsx` 口径 52 处里只有 46 处不是注释行。建议工具侧加注释过滤；在此之前该数字只能看**趋势**
      （本轮全仓 accessKey 22 → 26）。另：TOP #2 判据里的「缺口 < 46」在本轮开工时**已经不成立**（HEAD 实测约 50），
      不是本轮改动造成的。
- [ ] **TOP #5 发 v1.0.21** 仍待办：先读 `tools/loop/logs/round-138-gates.md` 的实际 `N/N` 校正 `RELEASE-NOTES-v1.0.21.md` 里的数字再打包。

---

## round-141 结算（`FAILURES.md` 非空 → 本轮唯一任务：修 round-140 的两条门禁红）

- [x] **`ui-v121.cjs` 11/12 → 14/14**：**环境变化造成的假红，产品无罪**。本机给 Gprinter GP-1324D 装上
      `Generic / Text Only` 驱动后 `USB001` 上出现了打印队列（round-139 门禁 13:33 仍 12/12，round-140 门禁 14:19 变红），
      而原断言把「该端口上没有队列」当成了恒定前提。**产品两条分支都对**（`commandTransport.ts` 的 `writeRawToWindowsQueue`），
      改的是**断言的取期望方式**：按 `listPrinters().port`（DIFF-91 增补的真实端口）判定分支再要求对应输出
      → **12 → 14 条，两条分支都钉住**（原来只覆盖「无队列」一种），强度不降。
      证据：`parity/FAILURES.md`（含实测输出原文）+ 本轮 `ui-v121` 14/14。
- [x] **`ui-v107.cjs`：单跑 3 次全 28/28 PASS，不可复现 → 不改断言**。round-140 的产品 diff 与它覆盖的
      B-02/B-05/B-44/B-45 无交集；时间线上唯一可疑的是门禁时段的资源争用（验收方刚跑过真实打印 + 本机正在装驱动）。
      没证据就不猜着改；改为补**取证能力**（下一条）。
- [x] **补掉「失败断言原文查不到」的取证盲区**（验收方 round-205 提过）：`run-regression.ps1` 新增 `Write-FailureLog`
      —— 失败脚本**完整输出**落盘 `tools/loop/logs/fail-<脚本>-<时间戳>.log`（gitignore），runner 异常路径附
      `electron stderr` 末 60 行；顺手把 **node 输出被按 GBK 解码**（中文断言名全成「鎺㈤拡」，round-140 门禁日志实测）
      改成跑脚本那一小段切 UTF-8 再还原。`runner-safety.test.cjs` **+6 条**静态断言（18 → 24）。探针实测落盘可读。
- [x] **matrix 证据同步**：3 行引用 `ui-v121` 的计数/结论已更新（`A-204` 10/10 → 14/14、`D-24` 补「有队列」分支已实测、
      `D-25` 12/12 → 14/14）。

### 本轮新登记的缺口 / 待办

- [ ] **`ui-v107`（以及其它拖拽/指针类脚本）的门禁抖动仍未定位** —— 本轮只能证明「单跑稳定、不可复现」。
      下一次全量门禁若再红，**直接看 `tools/loop/logs/fail-ui-v107-<时间戳>.log`**（有失败断言原文），
      再决定是产品竞态还是资源争用；**在拿到那份日志之前不要改它的断言**。
- [ ] `App.tsx` 的 `printPrinterInfo` 在**应用启动时**就会走一次 `listPrinters()`（主进程会拉起一次 PowerShell
      `Win32_Printer` 查询，本机实测 **~616ms**）。它只在「文件→打印」对话框里用到，理论上可以改成**打开对话框时再取**。
      本轮**没改**（无证据指向它是抖动原因，且改了会牵动 `ui-v142`/`Check-PrintDialogFields` 的等待口径）——
      登记在此，留作下次排查抖动的候选之一。
- [ ] 仍待办：TOP #2 文本页 / #3 字体页对齐、#5 发 v1.0.21（**先读** `round-138-gates.md` 的实际 `N/N` 再校正说明里的数字）。
...
**实际修的**（`app/scripts/ui-v81.cjs`；13 条断言表达式**一字未改**）：

**① 建文档竞态（最可能的真凶）**：原流程是 `^{n}` → `sleep(350)` → `if (向导存在) { 驱动向导 }`。
向导若在 350ms 内没渲染出来，`if` 为假 → **整个向导分支被静默跳过** → 编辑器永远打不开 →
`throw 'editor did not open'`（**exit 2**）。它与失败前状态无关：慢一拍就 100% 全挂，
且不由任何断言反映 —— 正好解释「单跑 13/13 / 门禁红 / 日志里只有脚本名没有断言明细」三件事。
改成先 `waitFor`「向导**或**编辑器」任一就绪（**保持原容忍度：两条路都算数**），再按实际出现的那条走。
另：「关闭」按钮改为先等出现再点；「管理」按钮原 `clickText('管理')` 的返回值被忽略（找不到就静默往下走），现改为等不到就带页面文本报错。
...
- [ ] **【系统性、优先级高】同一竞态还存在于约 55 个 UI 脚本里**（本轮只修了 `ui-v81.cjs`，其余**未动**）——
      这些脚本都是同一个写法：`^{n}` → **固定 `sleep(320~420ms)`** → `if (await evaluate('!!document.querySelector("[data-testid=template-wizard]")')) { 驱动向导 }`。
      与 ui-v81 完全同源：**向导慢一拍，`if` 为假 → 向导分支被静默跳过 → 后续步骤对着错误的界面跑**，
      表现为"某个脚本在门禁里红、单跑却全绿"（正是 round-136 的形态）。
      逐个复核现状（本轮实测，括号内为行号）：`ui-v100`(92/216)、`v102`(116)、`v103`(170/383)、`v104`(106/150/191)、`v105`(119)、
      `v106`(153)、`v107`(135)、`v108`(111)、`v109`(241)、`v110`(93)、`v112`(149)、`v113`(99)、`v114`(85)、`v115`(107)、
      `v116`(114)、`v117`(74/94)、`v118`(98)、`v119`(96/227)、`v122`(159)、`v123`(109)、`v124`(99)、`v125`(124)、`v126`(114)、
      `v127`(111)、`v128`(107)、`v129`(71)、`v130`(65)、`v131`(90)、`v132`(102)、`v134`(112)、`v135`(145)、`v136`(130)、
      `v138`(103)、`v139`(95)、`v140`(93)、`v70`(59/115)、`v71`(114)、`v72`(72)、`v74`(68)、`v75`(69)、`v76`(83)、`v77`(93)、
      `v78`(70)、`v79`(95)、`v80`(67)、`v83`(34)、`v84`(71)、`v85`(74)、`v86`(72)、`v87`(64)、`v88`(91)、`v89`(81)、`v91`(117)、
      `v92`(95)、`v95`(133)、`v96`(110)、`v99`(89)。

      复核命令：`grep -n "data-testid=template-wizard" app/scripts/ui-v*.cjs`

      **建议的统一改法**（照 ui-v81，**保持原容忍度**：向导与编辑器两条路都算数）：
      把「`sleep(N)` + `if` 判断」换成「**先等到向导或编辑器就绪**，再按实际出现的那条走」：
      先用 `waitFor('!!document.querySelector("[data-testid=template-wizard]") || !!document.querySelector("canvas.upper-canvas")', 15000)` 等就绪，
      等不到就 `throw` 并带上 `document.body.innerText.slice(-800)`；就绪后再 `if (向导存在) { 原向导分支 }`。

      **不建议一轮全改**（55 个脚本无法在单轮预算内逐个实跑验证）；建议**分批**（每批 8–10 个 + 单跑该批验证），
      或先给 `run-regression.ps1` 加一个共享 helper 再逐批迁移。**优先度：高** —— 它是"门禁随机红一个脚本"的主要来源，
      而 `consecutiveFail` 累到 3 会触发回滚（回滚本身还有丢提交的风险）。
...
- 产品：新增唯一转换点 `app/src/shared/mfcCaption.ts` 的 `displayMfcCaption()`（MFC 语义：`&&` → 一个 `&`；单个 `&` 是加速键标记，**不显示**），`FormField`（全应用字段标签的唯一原语）与 `GS1/EAN 128(&U)` 内联复选框标签统一走它。源码**仍保留控件树 dump 原文**（取证依据不丢），只在渲染时去掉标记。
- 断言：新增 `app/scripts/ui-v139.cjs`（**7/7 PASS**，已注册 `run-regression.ps1`）+ `app/scripts/mfc-caption.test.ts`（6 条，`npm run test:mfc-caption` 已接入 `npm test`）+ `architecture-check.cjs` 的静态不变量（源码出现字面量 `(&X)` 即红，注释除外——已用临时违规文件验证「加进去就红、删掉就绿」）。
- 连带迁移：15 个 `app/scripts/ui-v*.cjs` 的**代码位置**断言文案 `(&X)` → `(X)`（87 处机械替换，已用 diff 审计确认「每处只是少了一个 `&`」）；注释里的 dump 原文保持不变。复跑 `ui-v134.cjs` **19/19 PASS**。
- 台账：`parity/diffs.md` 新增 `## DIFF-83`（状态 / 证据 / 处置 / 断言 四要素齐）；`parity/matrix.md` 的 B-69/B-70/B-71/B-72 四条证据列补本轮。

**本轮新发现的缺口（下一轮候选）**

1. **`tools/parity/Verify-BarcodePage.cjs` 的 CLONE 口径已过期** ✗——它仍断言复刻版**修复前**的 `(&B)` 形态，产品修好后该条会转红。按工装自己的双口径设计（CLONE = 现状回归 / REAL = 真机口径），需把 `CLONE_LABELS` 更新为不带 `&` 的形态。**工装归验收方维护，本轮未改**，已写进 DIFF-83 的「遗留提醒」。
2. **全量 `test:ui` 未跑**（本轮时限内只单跑了 `ui-v139` 与 `ui-v134`）→ 另 14 个被机械迁移过的脚本（ui-v56/77/78/106/125/126/127/128/132/133/135/136/137/138）需下一次全量门禁覆盖。**风险如实说明**：改动是机械的「去掉一个 `&`」且已 diff 审计，但我没有逐个实跑。
3. **键盘加速键尚未接线**：`acceleratorOf()` 已备好但未接 `accessKey` / 快捷键派发——真机上这些 `(&X)` 是**可用**的加速键，复刻版目前只是不显示。属「原版有但受限」，需先取证真机按 Alt 后的实际跳转行为再决定是否实现。

---

## round-134 结算（P4 四件套：修好证据列里「引用写法」导致的可复核性丢失）

**起因**：本轮开工时 `parity/FAILURES.md` 非空 → 按其定性，round-133 门禁的 9 项失败是**验收方 node_modules 事故**（junction 被递归删除），
非产品回归；本轮先复核工具链已恢复（`npm run typecheck` exit 0、全部门禁 PASS），**未改任何产品代码**。

**本轮做的**：读 `tools/parity/survey-evidence-coverage.cjs` 的分类器后定位到一个**系统性可复核性缺口**——
普查是**按证据列里出现的文件名**判定的，而矩阵里有 35 行的断言引用只写了脚本号（如 `ui-v52「未连库数据库菜单禁用规则正确」`）、
**漏了 `.cjs` 扩展名**，于是这些行被普查判成「无自动断言」。这些引用指向的脚本**真实存在且确实断言了这些行**
（已逐条核对，例如 `ui-v52.cjs:217-221` 的「数据库菜单可打开」「未连库数据库菜单禁用规则正确」），
所以这是**引用写法问题，不是断言缺失**——补扩展名即可，**不改动任何断言内容、不新增任何主张**。

- [x] 35 行的证据列补上 `.cjs`（A-54~A-56、A-69~A-81、A-85、A-170、A-227~A-237、B-51、B-95~B-98、C-53）。
- [x] 效果（`node tools/parity/survey-evidence-coverage.cjs`）：**四件套齐 71 → 80**、有自动断言 491 → **520**（85.4%）、A 模块 四件套齐 17.8% → **21.0%**。
- [x] 台账：`parity/diffs.md` 的 DIFF-77 补「断言 / 状态」两行（验收方 round-165 明确要求「已修条目要指向机器可复核的断言」）；
      `node tools/parity/audit-diffs.cjs` 缺项 42 → **41**。

### ⚠️ 本轮事故与规程（务必遵守，防再犯）

- **`parity/progress.md` 是「最新一轮置顶」的累积日志（约 6700 行），绝不能用整文件覆盖写** ✗ ——
  本轮误用 Write 覆盖，删掉了 6689 行；已用 `git show d11400a:parity/progress.md` 恢复并在顶部**前置**本轮段落（见提交 `fa030a6`）。
  **规程**：写 progress.md 一律「先读首行 → 用 Edit 在标题行前插入」，或读全文后**拼接**再写；改完用 `wc -l` 核对行数不应显著减少。
- **同一事故还吞掉了开工时工作区里 progress.md 的「未提交改动」** ✗（session 开始时 `git status` 显示 `M parity/progress.md`）——
  该改动没有进过 index，git 对象库里没有副本，**无法恢复**；按本仓库惯例它多半是上一轮 driver 追加的 round-133 段落。
  对不起：这是一次不可逆的小损失，特此记录。

### 本轮发现的新缺口 / 未完成项（下一轮候选，按优先级）

- [ ] **`audit-diffs.cjs` 仍有 41 条缺项**（多为 DIFF-19~DIFF-59 的早期条目，缺的常是「处置 / 证据」关键词）。
      它们**多数是启发式误报**（例如 DIFF-19 的证据其实就是帮助页 `label_object_page_text.html` ↔ 代码逐项比对，属"纯帮助口径条目"），
      但按要求**应在条目里写明**四要素。建议下一轮**分批**（每轮 10~15 条）逐条补 `- 状态 / - 证据 / - 处置 / - 断言：` 四行，别一次性重写。
- [ ] **B-74 / B-75 / B-72 三条仍差一件**：它们的界面是「条码属性 → 条码」页的 `条码特殊选项`（GS1/EAN 128 + 字符集）与
      `供人识读字符`（字符模板）。库里现成的并排图 `parity/review/cmp-propsbarcode-r124.png` 我只看到**真机侧**画全了这两组，
      **复刻版侧该两组在对话框滚动区下方、未入画** → 按「不确定就不引用」的口径**本轮没有引用它**（不凑数）。
      要补齐就得**重出一张把这两组滚进可视区的并排图**（出图归验收方），或改用复刻版侧截图 `clone-propsbarcode-r124.png` 同款重拍。
- [ ] 同上：`B-64`（字体页字段）差**复刻版证据**，候选 `parity/reference/maxlabel/clone-propsfont-r151.png`——
      **未引用**：本轮没有逐像素确认该图确实拍到了「字体宽度缩放倍数 / 字间距」两项。下一轮先看图再引。
- [ ] DIFF-70（打印输出里有没有孔）仍**待取证**，路径不变：打印对话框 → 预览(V) → 再放一个满标签大矩形确认输出非空。

## round-133 结算（菜单项集与顺序对标真机：DIFF-65 收口 / DIFF-80 复核 / 云马通族补行）

- [x] **DIFF-65 收口（已修）** 工具(T) 菜单按真机**菜单资源**改正：移除真机没有的 RFID 项、把 `数据(D)` 提到 `表格(G)` 之前。RFID 能力未丢（工具栏入口保留）。
- [x] **DIFF-80 复核收口** 真机运行时实拍只有 5 项、无 `服务器...` → 记为「原版有但运行时不显示」，复刻版不实现（不造猜测版界面）。
- [x] **A-64/A-65 台账疑问**（验收方 round-163）：确系**帮助文字**，行标题已注明「（帮助文字；真机菜单无此项）」，并补真机图 r162-menu-09-window.png。
- [x] **矩阵补「云马通菜单」一族 4 行**（A-273~A-276），四件套齐：真机 r162-menu-07-cloud.png + 复刻图 + 并排图 cmp-menu-cloud-r162.png + ui-v138.cjs。矩阵 605 → **609** 条，100%。
- [x] 新增 `app/scripts/ui-v138.cjs`（**11/11 PASS**，已注册 run-regression.ps1）。

### 下一轮候选

- [ ] 工具菜单顺序改动后，**复刻版工具栏** `数据`/`表格` 两按钮的相对顺序仍是「表格在前」——真机工具栏按钮顺序**无直接证据**（exe 里没有 `选择工具：` 类 tooltip 串），未凭推理改。取证手法：真机编辑器截图放大工具栏图标区逐个数。
- [ ] 工具栏 RFID 按钮是否也该加「复刻版扩展」标记（现 title 仍为 `选择工具：RFID`）。
- [ ] DIFF-70（打印输出有没有孔）仍待取证。

## round-132 结算（选项/账户菜单项集与入口取证 —— DIFF-80 / DIFF-81 / DIFF-82）

依据：真机安装包 `LabelShop.exe` 的 CU16LE **菜单字符串表**（偏移 17401836 / 17446854，含选项/账户菜单全文）
+ 帮助 `menu_option.html`、`menu_help.html`；断言 `app/scripts/ui-v137.cjs`（6/6 PASS）。

- **DIFF-81（结论：复刻版正确，保留）** 验收方 round-158 的「复刻版选项菜单多出第三项 `电子称`」**不是差异**：
  真机菜单资源里 `选项(&O)` 就是 `系统选项(&C)... / 应用程序外观(&A) / 电子称` 三项（逐字一致，`电子称` 亦无加速键），
  帮助 `menu_option.html` 同样把「电子称配置对话框」列为该菜单条目。本机真机截图只有两项 = 该机**未接电子称**（按硬件条件显示）。
  → **未删**，并补 4 条断言钉住。
- **DIFF-82（已部分收敛）** `账户(A)→登录...` 原先在服务器不可达时**完全静默**；现改为把失败写进状态栏（`App.tsx` 菜单依赖改 `() => openCloud(setStatus)`）。
  **仍受限**：原版是应用内「登录 LabelShop」窗口（帮助 `menu_help.html` + 安装包字符串 `请使用云马科技账号登录服务器`/`请输密码`），
  复刻版走外部云服务窗口 → 按「原版有但受限」登记，不实现猜测版界面。
- **DIFF-80（新登记，未实现）** `账户(A)` 菜单**缺真机有的 `服务器...` 一项**：安装包菜单资源里
  `登录... / 注销... / 账号和授权管理... / 试用管理... / 服务器... / 演示和体验...`，帮助 6.37 尚无此项（6.37→6.39 新增）；
  同文件另有 `授权服务器设置` / `连接到授权服务器` / `连接密钥：` / `新增服务器连接成功` 等配套资源 → **是真缺口，不是等价替代**。
  **本轮不实现**（点开后的形态、`确定` 后的落点、与系统选项里单一服务器地址字段的关系均未取证）。
  下一步取证手法：真机 `账户(A)` 菜单截图 + `Read-LabelShopDialogTree` 树 dump，同 DIFF-63。
...
- [x] **蓝竖条只覆盖标题行**（真机竖条高 32 物理 px ≪ 整条文章 69）→ `border-left` 从整条文章移到**标题本身**（精修后落到 `.start-article-heading a`，见下）。
- [x] 断言：`app/scripts/ui-v54.cjs` **19/19 PASS**（11 → 19，新增 8 条，全部为**真机原文数值级**断言）。
- [x] 证据：复刻图 `parity/reference/maxlabel/r130-start-top.png` 与 `r130-start-bottom.png`；并排图 `parity/review/cmp-start-r130.png`。

### 本轮新发现的教训（已纳入规程）

- [x] **「真机资源原文」优先级高于截图量测**：本轮先按 `92-00-startup.png` 逐像素量测反推出一版修复，
      随后在 `parity/reference/labelshop/START-PAGE-SPEC.md` §3.2 找到验收方抄录的**原版 CSS 原文**（default.html 行 77-85）
      —— 一比对，量测反推的 5 处里 **4 处方向就错了**（竖条挂载点、条间距来源、日期间距、摘要缩进、红点字号）。
      → **规程**：动手改样式前，先 grep `parity/reference/labelshop/*.md`（尤其 `*-SPEC.md`）看有没有原文；
      有原文就**逐字照抄**，量测只用于交叉验证。
...
- [x] **蓝竖条绝对高度已收口**：真机 `dt b { line-height:22px; border-left:4px solid #0099ff; display:inline-block }`
      → 竖条挂在**标题本身**上、高 22px。精修后复刻版实测竖条高 **33 物理 px**（真机 32）。
...
- [x] Data Matrix 的「纠错级别（仅 ECC200）」只读项：真机该页无此控件（`probe-sym-datamatrix-values.txt` 仅有 `X 尺寸(&X):` / `字符编码:` / `符号版本:`），已按「复刻版扩展（只读）」登记在 DIFF-77；复刻版禁用展示由 `ui-v135.cjs`/`barcode-spec.test.ts` 钉住。
- [ ] DIFF-70（打印输出有没有孔）仍待取证，路径：打印对话框 → 预览(V)。
...
- [x] Data Matrix 的「纠错级别（仅 ECC200）」只读项：真机该页无此控件，按 DIFF-77 的三态结论收口为「原版有但受限：ECC200 固定、复刻版只读展示」，证据 `probe-sym-datamatrix-values.txt` / `ui-v135.cjs`。
- [ ] DIFF-70（打印输出有没有孔）仍待取证，路径：打印对话框 → 预览(V)。
...
- [x] `ui-v134.cjs` **产品缺陷**：条码页「供人识读字符」组被渲染在「条码特殊选项」**之前**，与真机 y 序相反
      （`probe-60-barcode-props-tree.txt` L147-149：`尺寸`521 → `条码特殊选项`665 → `供人识读字符`821）→
      已把该 fieldset 移到特殊选项之后、页尾「颜色:」之前。`ObjectPropsDialog.tsx`。
- [x] `ui-v56.cjs` / `ui-v77.cjs` / `ui-v106.cjs`：断言查的是被真机证伪的旧文案 `GS1/EAN-128`
      → 改真机原文 `GS1/EAN 128(&U)`（L128/L181）+ `字符集(&C):`；五档字符集选项仍逐项比对，强度不降。
- [x] `ui-v77.cjs` `B-69`：**round-124 的遗留**（`条宽比(&W):` 按码制条件渲染后没有同步该断言）→
      改成先切 Code 39 再读 7 档；并拆成 `B-69`（X 尺寸 1–1000/步长 1/mil/常规页高度）+
      `B-69a`（Code 128 无该行、Code 39 有且 7 档）两条，断言数 7 → **8**（只增不减）。
- [x] `ui-v134.cjs` 的「码  高(&H):」字面量：改用与产品一致的 `U+00A0 + 空格`
      （真机 dump 是两个 ASCII 空格，但 HTML 里连续空格会被合并，NBSP 才能让渲染与 innerText 都保持两格宽）。
- [x] 复跑：`ui-v56` **11/11**、`ui-v77` **8/8**、`ui-v106` **33/33**、`ui-v134` **19/19**；
      门禁 `typecheck / architecture / editor / geometry / history / print / render / workspace / build` 全 exit 0；
      `Check-Matrix.ps1` exit 0（605/605）。

### 本轮新发现的缺口（下一轮候选）

- [ ] **全量 `test:ui` 未跑**（本轮改了 `ObjectPropsDialog.tsx`，属 UI 相关文件）。已单跑覆盖 4 个脚本；
      若还有别的脚本隐含「供人识读字符组在特殊选项之前」的假设，只有全量跑才会暴露 → 建议下一轮门禁跑全量。
- [ ] DIFF-76 第 5 项仍未动：`码  高` 真机 `10.00` vs 复刻版 `12`；`X 尺寸` 真机显示 `10.00 mil`（两位小数）。
      需先分清是「新建条码的默认尺寸」还是「下拉项文本格式」，否则容易只改显示却改了输出（来源：`probe-60-barcode-props-tree.txt`）。
- [ ] 同 dump 还暴露真机「尺寸」组有一行 `字符编码:`（Code 128 下不可见），可见码制与选项集待取证。
- [ ] DIFF-70（打印输出有没有孔）仍待取证，路径：打印对话框 → 预览(V)。

## round-126 结算（B-71 条码属性「条码」页：按真机控件树重构 + 消除重复入口）

判定依据：`parity/reference/labelshop/probe-60-barcode-props-tree.txt`（真机 Code 128 条码的递归控件树，逐行带可见性与对话框内坐标）。

- [x] **B-69 / B-70 / B-71 / B-72 四条同模块收口**（DIFF-76「仍未收口」五项中的 1～4 项）：
  - ① 补出真机三个**分组框** `尺寸`(928,521) / `条码特殊选项`(928,665) / `供人识读字符`(928,821)，字段按真机归组（原先全平铺，只有一个自造的「条码特殊选项」fieldset）；
  - ② 字段原文+加速键：`位置(&P):` / `垂直偏移(&O):` / `对齐方式(&A):` / `字符集(&C):` / `缩减量(&M):` / `GS1/EAN 128(&U)`；
  - ③ **`字符模板(&T)` 补进 `供人识读字符` 组**（真机位置 (961,959)），绑的仍是正式模型字段 `charTemplate`（`lsdxImport` 也解析它）——不是假实现；
  - ④ 「数据源」页原先**又渲染了一份**同样的 `humanPosition/humanAlign/humanOffsetMm` + 字符模板（同一状态两个入口）→ 按「真机数据源页无这些控件」移除（dump 已核）；`ui-v71` / `ui-v78` 两条原查数据源页的断言**迁移到条码页并加严**（加了分组框名与加速键原文），强度不降；
  - ⑤ 自造的「对齐」字段（`barcodeAlign`，真机**条码页无**此控件；真机的「对齐」是「常规」页的分组框，装 `水平(&W):`/`垂直(&T):`）→ 移入 `data-testid=barcode-extensions` 的**复刻版扩展**区并加图例保留（它驱动可变数据打印摆位，`rendering/fabricObjects.ts` 在用，不静默删）。
- [x] 断言：新增 `app/scripts/ui-v134.cjs`（已注册 `run-regression.ps1`）**19 条**（分组框整数组全等 / 组内字段原文整数组全等 / 四个供人识读控件在整页各恰好 1 个 / 数据源页无重复项 / 扩展区图例 / 「尺寸」组 Code 128 无「缩减量」/ GS1 与字符集原文 / 颜色色块仍在）。
- [x] 门禁：`typecheck` / `test:architecture|editor|geometry|history|print|render|workspace` / `build` 全 exit 0；`Check-Matrix.ps1` exit 0（605/605）。
- [ ] **未跑**：本轮 `test:ui` 全量**未跑** —— 启动时独占锁被验收方的 UI 实例占着（`maxlabel-ui-regression.lock` 被拒），故只静态写完脚本，**`ui-v134.cjs` 尚未实跑过**，请验收方在下一轮门禁一并覆盖（预期 19/19）。
- [ ] **DIFF-76「仍未收口」第 5 项**（默认值差异：`码  高` 真机 `10.00` / 复刻版 `12`；`X 尺寸` 真机 `10.00 mil` / 复刻版显示 `10`）**本轮未动** —— 需要先确认真机这两个默认值分别属于"新建条码对象的默认尺寸"还是"下拉项文本格式"，避免只改显示格式却改了实际输出。
- [ ] 同 dump 还显示真机「尺寸」组末尾有一行 **`字符编码:`**(961,611)（Code 128 下不可见 `[ ]`）→ 待核实它是哪些码制可见、选项集是什么（复刻版的 `字符编码` 在 QR/DataMatrix 的扩展字段里）。
...
- [ ] **B-54**：已补真机常规页截图 `probe-b54-general-r125.png` 与控件树 `probe-probe-b54-general-r125-tree.txt`（颜色固定颜色、位置/对齐/其它字段清晰可见）；仍缺当前构建同态复刻图/并排图。先前误截图已移出证据目录；不能引用 `cmp-props-r113.png`（其左侧是条码页）。
...
- [x] 重建 `parity/P4-四件套工单.md`；本轮新增文件经 `node tools/parity/check-evidence-files.cjs` 校验存在，矩阵 `605/605`，`Check-Matrix.ps1` exit 0。
- [x] 真机顺手复核新建标签/打印入口：用既有 `LabelShopCtl.ps1` 走到「选择标签格式」和右侧「打印」；打印仍先弹「登录 LabelShop」，取消后进入「将打印输出另存为」，本轮未把空白 PDF 误判成孔洞输出结论。
- [ ] **DIFF-70 仍待取证**：需要通过「打印对话框 → 预览(V)」或得到有绘制算子的实际输出，确认打印结果是否包含孔；当前登录/空白 PDF 证据不足以实现或修改产品行为。
- [ ] 后续 P4 缺口按新工单推进：系统设置仍缺 A-184/A-185/B-71 的当前复刻证据；并排图缺口为 A-43/A-121/B-54/D-02/D-34/D-35；驱动属性表「首选项/自定义命令」两页仍未取得真机控件树。

---

## round-122 结算（打印机属性收口复核 + DIFF-72 唯一控件断言）

- [x] 复核在途打印机属性改动：`PrinterSettings.tsx` 底排为真机原文 `确定 / 取消 / 帮助`，`恢复默认` 移入明确标注的复刻版扩展区，`指令编码` 保留并标注为复刻版扩展；`ModalHost.tsx` 的帮助入口可打开帮助主题。`ui-v133.cjs` 已注册，**8/8 PASS**。
- [x] DIFF-68 台账去除已过期的“形态待取证”结论：真机 `r121-prn-switches-before.png` → `r121-prn-switches-after.png` 证明 `整页反相打印` 为保持勾选的复选框且不弹窗；`单页任务模式` 同形，`镜像输出` 在页式打印机态隐藏。复刻版现有 checkbox 形态不改。
- [x] DIFF-72 回归断言加严：`ui-v125.cjs` 对条码页 `barcode-color` 与常规页 `color-change-mode` 均按**恰好 1 个**计数，并保留真机文案/选中值校验；**19/19 PASS**。
- [x] 刷新 `parity/P4-四件套工单.md`；当前普查为全矩阵 605 条、四件套齐 20 条，工单生成时间更新为本轮。
...
- [x] **第 1 项**：`分享(I)...` → 真机原文 **`分享(T)...`**（`app/src/renderer/src/features/commands/labelShopMenus.ts`）。
      并排证据 `parity/review/cmp-menu-r119.png`（同态，两侧都有文档）；`T` 未被文件菜单其它项占用。
      断言：`ui-v124.cjs` 新增「加速键为真机原文 (T) 且 T 未被其它项占用」→ **12/12 PASS**；
      `ui-v51/v52/v83/v90` 里引用旧文案的断言同步改名（强度不变）。
- [x] **第 2 项**：打印机属性「端口」页 `类型(T)` → 真机原文 **`类型(I):`**（`PrinterSettings.tsx`）。
      断言：`ui-v120.cjs` 新增「类型字段标签为真机原文「类型(I):」」——读 FormField 真正渲染的 `<label>`，
      原来只断下拉选项文本、断不出字段名。
- [x] **第 3 项**：端口页**去掉空云盒的红色阻断校验**。`shared/domain/printer.ts` 的 `portConfigError()`
      只对**标准 TCP/IP** 要求主机名必填；云盒空主机名 = 真机的「未检测到云盒」正常态（同态图里页面无任何报错）。
      断言：`ui-v120.cjs` 新增「空云盒时不报错且「保存」可用」→ **16/16 PASS**；
      `printer-catalog.test.ts` 该用例按新语义重写并**加严**（5 条 → 8 条，新增「格式错仍报错」「TCP 仍必填」）。
- [x] **第 6 项**：`界面语言(L):` 按「**原版有但受限（仅实现简体中文）**」记入 `parity/diffs.md` 的 DIFF-71 补记，
      **不列假选项**（复刻版没有繁体/英文语言包）。
- [x] **台账**：新增 `parity/diffs.md` **DIFF-74**（端口页两处，含断言清单与"为什么空云盒不该阻断"的口径）；
      `matrix.md` 的 A-39 / A-177 / A-257 / D-22 / D-23 / D-28 证据列同步；`Check-Matrix.ps1` exit 0（605/605）。
- [ ] **第 4 项（待核实，未动）**：端口页的 `指令编码` 一项 —— 真机该页没有；要先查真机「首选项」页是否也没有。
      现有 dump（`probe-14-printer-props-combos.txt` / `probe-15-cloudbox-combos.txt`）只覆盖**端口页的下拉**，
      **没有首选项页的控件树** → 需要真机把 `Gprinter GPL-N (203 dpi) 属性` 切到首选项页再 dump。
- [ ] **第 5 项（待核实，未动）**：底排按钮形态。`PROBE-round106.md §5` 记的属性表按钮集是
      `刷新USB端口 / 设置 / 确定 / 取消 / 应用(A) / 帮助`，与验收方 round-136 说的「`确定/取消/帮助`」相差一个
      **`应用(A)`** —— 需要先确认真机从**别的入口**（打印面板「设置」 vs 模板属性）进来是否也是这三个按钮，
      以及 `应用(A)` 是可见还是隐藏控件。**结论明确前不改。**

- [ ] **已知的既有红灯（不在门禁范围内）**：`ui-v51.cjs` 现在是 **11/19（8 FAIL）**，与我的改动无关 ——
      `tools/loop/logs/round-104-settle.err.txt` 已记录同一现象（同样 11/19、同样 8 条 FAIL，含
      「选择标签格式进入编辑态」「有文档顶层菜单为12项且顺序正确」等）。`run-regression.ps1` 注释写明
      「ui-v48～ui-v51 是早期脚本，长期未登记进门禁」，所以门禁看不到它。**本轮只按改名同步了其中两处文案断言，未改断言强度。**

---

## round-119 结算（D/打印模块：DIFF-70 再取证，仍受登录/空白输出阻断）

- [x] 复验上一轮唯一门禁失败：`MAXLABEL_UI_SCRIPT=ui-v108.cjs npm run test:ui` → **8/8 PASS**；该失败修复已在 HEAD，未改弱断言。
- [x] DIFF-70 重走既有真机配方：`孔洞=圆洞 / 尺寸=20` 的编辑器孔洞可见；点击右侧“打印”先弹“登录 LabelShop”，取消后可进入“将打印输出另存为”。
- [x] 保存并独立核验 `parity/reference/labelshop/round119-hole-print.pdf`：1 页 A4、835 字节、无文本/图片/绘制路径，渲染为纯白页。
- [ ] **DIFF-70 仍未收口**：本轮只证明“原版有但受限：需登录”与“未登录取消后的产物为空白”，仍不能判断打印输出是否包含孔。完整记录：`parity/reference/labelshop/PROBE-round119-DIFF70.md`。
- [ ] 同模块遗留 DIFF-68：打印机页 `整页反相打印` / `镜像输出` / `单页任务模式` 的点击后控件形态仍需真机状态证据，不按 `class=Button` 猜测。
...
| # | 条目 | 处置 | 证据 |
| --- | --- | --- | --- |
| 1 | `test:ui` 失败 1 项：`ui-v108.cjs` 7/8 | **不是抖动**。根因＝round-116 提交 `fec35d6` 把「颜色变化模式」并入「常规」页 `颜色(&C):` 时，渲染条件从 `colorChangeEnabled`（=对象类型 **且** `printerSupportsColor`）降成了**只判对象类型** → USB 直连的普通条码标签打印机下颜色模式下拉仍然渲染（实测 7 项可选），与帮助 `getstart_color.html`／A-201「普通条码标签打印机无法选择彩色打印」冲突。**修法**：`ObjectPropsDialog.tsx` 该行条件改回 `colorChangeEnabled`；**未改任何断言、未降强度** | `parity/FAILURES.md`；`ui-v108` **8/8**；连带 `ui-v74` 10/10、`ui-v85` 7/7、`ui-v92` 11/11、`ui-v125` 19/19 全绿 |

### 二、队列第 1 项 DIFF-63：序列号面板的「重置」按钮 —— **已收口**

| # | 条目 | 处置 | 证据 |
| --- | --- | --- | --- |
| 2 | 真机「序列号设置」的 `重置` 按钮 | 真机**只有一个 `重置`，可见但恒禁用**（`[V] Button DISABLED`）；「重置初始值:」「立即重置」是**隐藏**控件、无用户可达路径。复刻版**此前两者都没有**（队列里"若有两个按钮"的前提不成立）→ 补上**单一** `重置`（`data-testid=serial-reset`），**渲染为 disabled、无行为**；两项隐藏控件**不落地**（口径同「应用(&A)」） | `PROBE-verifier-round88-DIFF63-serial.md`、`PROBE-round104-DIFF63.md` |
| 3 | 序列号区字段名缺加速键 | 逐字对齐真机原文 `类型(&T):` / `序列(&Q):` / `步长(&S):` / `重复(&E):` / `初始值来源(&R):` | 同上；`ui-v132.cjs` **5/5**（已注册 `run-regression.ps1`） |
| 4 | `重置` 的启用条件 | **仍未取证**（真机在两种初始值来源下都禁用）→ 复刻版保持禁用，**不猜条件、不编造点击行为**；已写进 `diffs.md` DIFF-63 收口表 | 同上 |

**本轮未做（结构债 / 待办，不阻断）**：
- 复刻版把序列号设置**内联**在「数据源」页，真机放在「高级选项」对话框里 → 版式差异（取值语义一致），登记在 DIFF-63。
- 队列第 4 项（DIFF-73 预览拼版网格）已由 round-116 落地；第 5 项 P4 四件套引用可继续按 `parity/P4-四件套工单.md` 推进。
...
「新增编辑/系统两页 + 页签断言收紧 + 不许静默删功能」一并做掉。

| # | 条目 | 处置 | 证据 |
| --- | --- | --- | --- |
| 1 | 页签结构（真机 4 页 / 复刻版 3 页且其一自造） | 改为 `常规 / 打印和数据库 / 编辑 / 系统`（逐字 + 顺序） | `probe-r112-sysset.md`；`ui-v103.cjs` 整数组全等断言 |
| 2 | 复刻版自造「标签」页（真机无对应物） | 删页签，6 项设置 + 云服务器地址移入「复刻版扩展」区（`options-extensions`），**不静默删除**；形状下拉同时按真机 UI 改 `方角矩形` | `diffs.md` DIFF-71 round-114 收口表 |
| 3 | 真机「编辑」页 | 新增；`表格操作` 分组 + `增删行列时，保持表格尺寸` 接真实行为（新建表格 `keepSize` 全局默认） | `objectFactory.ts` / `App.tsx`；`editor-operations.test.ts` 42 项 |
| 4 | 真机「系统」页 | 新增；`系统操作` 分组 + `恢复默认窗体布局` 接真实行为（重置 toolbarGroups/toolbarLayout） | 同上 |
| 5 | 真机「打印和数据库」页字段 | 加 `数据库` 分组，字段改真机原文 `默认使用多个数据库连接(M)` | 同上 |
| 6 | 断言强度 | 页签与常规页分组框由「过滤/包含」**改为整数组全等**；新增 5 条 DIFF-71 断言 | `MAXLABEL_UI_SCRIPT=ui-v103.cjs npm run test:ui` → **15/15 PASS** |

**本轮未做（已写进 DIFF-71「仍未收口」，不渲染空壳控件）**：真机「编辑」页的
`鼠标拖动时仅调整首行首列尺寸` / `鼠标拖动时仅调整末行末列尺寸` / `禁用鼠标拖动复制功能` / `使用宽松圈选模式`；
真机「系统」页的 `文档` / `授权许可` 两个分组；真机「打印和数据库」页的
`打印到文件` / `打印设置(S):` / `使用常规 Excel engine` / `查重打印`。
这些都缺**行为语义**（复刻版无对应实现），按「不许按猜测造功能 / 不许留 TODO 占位」的口径暂不落地。
其中 `禁用鼠标拖动复制功能` 与优先级 2 的「CTRL+拖动是复制还是移动」是同一个未取证问题，建议合并取证。

**下一步建议**：① 取证「编辑」页那 4 项的语义（真机点选后观察行为变化）；② 队列第 1 项 DIFF-70（打印输出里有没有孔）
改走「打印对话框 → 预览(V)」路线；③ P4 界面清单的前三条「只缺并排图」可直接引用 `cmp-choose-label-r112.png` 收口。

---

## round-110 结算（只落账，未写任何产品代码）

结算时 `HEAD = e4c5205`。核对上一轮（round-110）实际入库内容后确认：**实施成果已由本轮自行登记完毕**，
本次结算没有需要改状态或勾选的条目，实际做的是**校验**与**一处台账矛盾更正**。

**上一轮到底改了什么**（`git log` 逐条核对）

| 提交 | 时间 | 归属 | 内容 |
| --- | --- | --- | --- |
| `91c0cd7` | 13:56 | 实施者 round-110 | 真机打印链路取证 + 新工装 `Set-LabelShopField.ps1` + 登记 DIFF-70 |
| `f329fe1` | 13:56 | 实施者 round-110 | `progress.md` 记账 |
| `b948dc4` | 13:57 | 实施者 round-110 | 门禁记录更正（typecheck 已跑 exit 0） |
| `8d949ed` | 13:57 | **验收方** | 更正 round-47「本机不可达」——右侧面板「打印」不需登录 |
| `97f955f` | 14:04 | **验收方** | P1.5 施工单复核（`tools/loop/round-focus.md`） |
| `f41afd5` | 14:04 | **验收方** | `Verify-LabelFormatDialog.cjs` 加检查⑩（圆洞尺寸>0 时子路径=2、=0 时=1） |
| `e4c5205` | 14:26 | **验收方** | 新增 `tools/parity/Capture-CloneShot.cjs`（复刻版三场景出图，供 P4 四件套采集；不进回归清单、不占 `test:ui` 锁） |

→ 13:57 之后的四个提交均为**验收方产物，不含产品代码**，故不需要在 matrix/backlog 里登记为"已完成条目"。
两个新工装本身值得记一笔，别丢：`Capture-CloneShot.cjs` 是 P4「界面清单」并排图采集的**复刻版侧来源**；
`Verify-LabelFormatDialog.cjs` 的检查⑩把 round-44 的真机取证（真机会画孔）钉进了工装。

**门禁**（`tools/loop/logs/round-110-gates.md`，14:28）：**全部通过** —— `typecheck` / `architecture` /
`editor` / `geometry` / `history` / `print` / `render` / `workspace` / `build` 全 exit 0，
全量 `test:ui` **82/82 脚本全绿**（含新增 `ui-v131.cjs` 16/16），`parity:matrix` exit 0。
工作树里 `tools/loop/last-gates.md` 的改动就是这个门禁产物，本轮一并提交。

**校验结果**

- [x] **matrix 状态列**：`待核 0 / 部分 0 / 未实现 0 / 已实现 605`（100%），`Check-Matrix.ps1` **exit 0** → **没有可翻的条目**（不是漏做，是上一轮已登记完）。
- [x] **证据存在性实测**（本轮新加的校验）：解析 605 行证据列里引用的 `app/scripts` / `app/src` / `parity/reference` /
  `parity/review` / `tools/parity` 路径，**去重 173 个文件，磁盘缺失 0 个**。即"已实现"条目的证据不是空指针。
- [x] **backlog**：round-110 段的勾选与未收口项逐条复核，与 `diffs.md` / `progress.md` 一致，无错勾。
- [x] **门禁**：见上，exit 0。

**发现并更正的台账矛盾（本轮唯一实质性改动）**

- [x] **`diffs.md` DIFF-50 自相矛盾**：同一条号出现**两个条目**且状态对立 —— 上方写「观察项，未收口 → ⏳ 待定」，
  下方写「已收口 → ✅ 已修（round-57）」。这个矛盾还外溢到了 `tools/loop/round-focus.md` 的基线，
  把 DIFF-50 误列为「未收口差异」。已把上方那条改标为**「原始观察记录 —— 本条已于 round-57 收口，见下方同名条目」**
  并加结算更正说明（原始真机证据保留备查，不再作为待办）。
  **更正后 DIFF-50 的真实状态 = ✅ 已修（round-57，`ObjectPropsDialog` 的 `alignOptionsAvailable` + `ui-v125.cjs`）。**

**更正后的真实未收口清单**（以此为口径）

| 编号 | 事项 | 状态 |
| --- | --- | --- |
| DIFF-60 | 真机 EAN/UPC 条码页无「附加条码」「校验字符」下拉，复刻版按帮助保留 | ⏳ 观察项 |
| DIFF-63 | 序列号面板「重置初始值 / 立即重置」缺可显示重置组的模板/数据库状态 | ⏳ 待取证 |
| DIFF-65 | 真机「工具」菜单无 RFID 工具，复刻版有 | ⏳ 待取证 |
| DIFF-67 | `OptionsDialog.tsx:266` 形状下拉仍写 `直角矩形`，真机原文待取证 | ⏳ 待取证 |
| DIFF-68 | 标签格式设置→打印机页三开关的控件形态（按钮 vs 复选框）待取证 | ⏳ 待取证 |
| DIFF-70 | 真机打印输出里有没有孔（链路已通，产出空白 PDF） | ⏳ 待取证 |

**新登记（本轮只记不改，留待后续轮次处置）**

- [ ] **`diffs.md` 编号冲突 `DIFF-28`**：同一号被**两个不同议题**占用 —— ① 创建对象交互三处缺陷（已修，
  `ui-v116.cjs` 10/10）；② 工具栏「添加或删除按钮」下拉结构（round-91 取证 / round-92 已修）。
  两者都已收口，所以**不影响"未收口"口径**，但引用「DIFF-28」时有歧义。
  建议后续轮次把 ② 改号为 `DIFF-71` 并同步引用处（`grep -rn "DIFF-28" parity/ tools/`），本轮不动以免制造大 diff。
  （同类重号还有 DIFF-14 / DIFF-15，但它们的第二个块已显式标注「原始描述，保留备查」且状态一致，**不算问题**。）
...
- [x] **修 FAILURES.md**：round-107 全量 `test:ui` 唯一失败脚本 `ui-v90.cjs` 的 A-42 断言已修（14/14 PASS）。
  根因是 round-106/107 按真机原文把 `TemplatePropsDialog` 标签页字段改名后断言未同步；且 round-106/107 的 `-gates.md` 是旧日志，全量 UI 从未跑到，失败被拖后才暴露。
- [x] 顺带修掉该断言里的一处**误报源**：`textContent` 会把分组框 legend「标签」与字段「宽度(W):」连成「标签宽度(W):」，用整段文本判自造名会把**正确**的 UI 判成失败（验收方 round-105 也踩过同一个坑）。改为只取 `FormField` 渲染的 `<label>`。
- [x] 断言强度不降反升：4 个字串 → 6 个真机字段逐项命中 + 4 个自造名一个都不留 + 5 个分组框齐全 + 四页签逐项相等。
- [x] 新登记 `DIFF-67`：帮助 `label_page_label.html` 写「直角矩形」，真机 UI 写「方角矩形」——按「真机 > 帮助」取后者；源码注释同步注明该差异。
- [x] 新登记 `DIFF-68`：打印机页三个开关（整页反相打印 / 镜像输出 / 单页任务模式）的**控件形态**待取证（Win32 `BS_PUSHLIKE|BS_AUTOCHECKBOX` 与普通按钮同为 `class=Button`，dump 无法区分）。取证前不改。

### 待办（下一轮候选，按优先级）

- [ ] 真机取证 `OptionsDialog.tsx:266`「系统选项」里形状下拉的原文（DIFF-67 遗留），再决定是否同步改名 `直角矩形 → 方角矩形`。
- [ ] 真机取证打印机页三个开关的控件形态（DIFF-68）：点一次看是否保持按下 / 是否弹窗。
- [ ] P0 追加 6：真机到底在哪些场景画孔（光盘/圆洞格式、20mm 圆洞、打印到 PDF 三条路径）。
...
- [x] **③ 全量复跑确认 `ALL SCRIPTS PASSED (64/64)`**：**本轮有意不自行启动**——新增的独占锁会让「我起的全量」与「门禁的全量」互斥，门禁那条会直接 `exit 1` 报「拒绝并发执行」。这正是 round-99 的教训（上一轮在报告前启动全量没跑完就进尾声，门禁接着跑，两者并发造出 14 个假失败）。改由**门禁自身的 `test:ui`** 提供该证据；届时门禁日志末行应为 `ALL SCRIPTS PASSED (64/64)`。
  - **证据已取得（round-101 结算登记）**：round-101 门禁（`tools/loop/last-gates.md`，2026-09-17 08:03:47，HEAD `94402e4`）`test:ui` **exit=0、1312s、末行 `ALL SCRIPTS PASSED (64/64)`** —— 与 `run-regression.ps1` 登记表条数（实测 64 条：`ui-v49`/`ui-v50` + `ui-v52`…`ui-v113`）吻合，即**该次全量回归跑满 64 个脚本、汇总行正常打印、无整轮中止**。同次门禁 `test:architecture` 打印 **18 runner safety checks passed**（含新增的 ⑧ `$WaitSeconds` 断言），说明 ④ 的回归锁也在门禁内生效。
  - 该日志按「只保留末尾若干行」截断，`test:ui` 段可见的 `ui-v90`…`ui-v113` + 汇总行正属截断后的尾部。

### 未收口

- 中止是**间歇性**的（round-100 手工跑在 ui-v60 中止，而紧随其后的门禁跑满了 64 个并正常打印汇总行）。本轮修的是**两个已定位的根因**；③ 那条全量复跑的判据**已由 round-101 门禁满足**（见上，64/64 + 汇总行），故「整轮中止」当前**无复现**，且**本次运行后实测 `%TEMP%\maxlabel-ui-*` = 0 个、`electron.exe` = 0 个**——③④ 的自愈与异步竞态修法在真实全量跑上成立。仍**不能**宣称「中止已被证明消灭」：单次绿不足以证明间歇性缺陷消失，判据是**后续每轮门禁是否持续出现汇总行**。若后续**再次**出现「整轮中止」，新 runner 至少会留下 `runner 内部异常：...` 或明确的 `FAILED SCRIPTS:` 行，可据此区分是 runner 挂了还是断言失败；若连汇总行都没有，说明进程被**外部**强杀（不再可能是 runner 自己杀自己，因为已没有任何按 PID 杀进程的代码路径）。

## round-101 结算（只落账，未写代码）

round-101 以**超时结束**（agent 未及把成果登记完，`parity/progress.md` 末条仍为 round-100）。本轮逐份核对上一轮实际入库的改动后落账，**零产品代码改动**。

**开工核对**（`git log` / `git show --stat HEAD` / `git status --short` 实测）：round-101 入库三个提交——`0860ac5`（runner 修「整轮中止、无汇总行」：删 `Stop-ProcessTree` 改按 profile 匹配 + 汇总行进 `finally`）、`819adab`（落账 round-101 正文）、`94402e4`（HEAD，清理修 `Stop-Process` 异步竞态：抽出 `Get-ProfileElectronProcesses` + 轮询等进程真退出）。工作区只有 `tools/loop/last-gates.md` 一处未提交（门禁产物，属正常）。

### 一、矩阵（`parity/matrix.md`）：**无** `待核` → `已实现` 可翻转

实测 `powershell -File tools/parity/Check-Matrix.ps1` → **exit 0**，**605 条 = 已实现 605 / 部分 0 / 未实现 0 / 待核 0（覆盖率 100%）**。

- round-101 的改动**全部落在测试基础设施**（`app/scripts/run-regression.ps1` 的清理路径、`app/scripts/runner-safety.test.cjs` 的回归锁）与台账，**零产品代码改动**，`git show --stat` 显示它**根本没有触碰 `parity/matrix.md`**。矩阵里不存在与之对应的条目，故本轮**不改动任何矩阵条目的状态与证据列**——不做「为凑改动而补证据」的动作。
- 复核结论：**没有**任何条目的证据列因 round-101 而失真（runner 只影响「门禁怎么跑」与「profile 怎么清」，不影响「产品做什么」）。
- 顺带复核证据引用面：`ui-v109.cjs` 仍**不在** `run-regression.ps1` 登记表内（round-86 起的既定状态，A-207/A-208 因此保持 `部分`，见 backlog 未收口区），矩阵 A-207/A-208 的证据列写明了这一点，无失真。

### 二、`parity/diffs.md`：本轮**无**差异行可勾（已全收口）

实测 `parity/diffs.md` 的 **22 个 `## DIFF-` 标题**：除 `DIFF-14`/`DIFF-15`/`DIFF-16` 的「原始描述，保留备查」小节（各自上一行已有 `✅` 收口标题，无 `✅` 属**有意为之**）外，**全部**带 `✅ 已修/已收口`。表格形式的 `| DIFF-1 |` … `| DIFF-11 |` 11 行亦全部为 `✅`。**故本轮不动 `diffs.md`**。

### 三、`parity/backlog.md`：勾掉全量复跑确认（1 条）+ 更新未收口定性

- **[x] ③ 全量复跑确认**（round-101 正文里的最后一条未勾项）**已勾**，证据是 round-101 门禁本身：
  - `tools/loop/last-gates.md`，2026-09-17 08:03:47，HEAD `94402e4`，**全部通过**；`test:ui` **exit=0、1312s、末行 `ALL SCRIPTS PASSED (64/64)`**，与登记表实际条数一致（64 条），即**跑满、汇总行正常打印、无整轮中止**。
  - 同次 `test:architecture` **exit=0**、打印 **18 runner safety checks passed**（含 ④ 新增的 `$WaitSeconds` 断言），说明异步竞态修的回归锁确实在门禁内生效。
  - **本轮补测（全量跑完约 1 分钟后，2026-09-17 08:04）**：`%TEMP%\maxlabel-ui-*` 目录 **0** 个、`electron.exe` **0** 个 —— ③（清扫残留 profile）与 ④（等进程真退出再删）在**真实全量跑**（64 个脚本，此前每次都留残骸）上成立，而非只在单脚本验证里成立。
- **未收口定性改为「当前无复现，但不宣称已消灭」**：单次绿不足以证明**间歇性**缺陷消失，判据是**后续每轮门禁是否持续出现汇总行**；若再中止，新 runner 至少会留下 `runner 内部异常：...` 或明确 `FAILED SCRIPTS:` 行，若连汇总行都没有则说明被**外部**强杀（已无任何按 PID 杀进程的代码路径）。

### 四、剩余未登记 / 不确定的部分（如实列出）

1. **round-101 在 `parity/progress.md` 中没有条目**：`progress.md` 的轮次条目由循环控制者按上一轮 agent 汇报追加，round-101 超时未产出汇报，故末条仍为 round-100。本轮**未代写**（避免伪造 agent 汇报），round-101 的实际产出以 backlog 本节与其上方的 round-101 工作小节为准。
2. **「整轮中止」只是「未复现」而非「已证明修好」**：见上。门禁这次绿了 64/64，但它是间歇性缺陷（round-100 手工跑在 ui-v60 后中止、门禁却跑满），需要后续轮次继续观察。
3. **`parity/SCORECARD.md` 仍落后于矩阵**（其上仍是 `2026-09-17 04:11:45` / HEAD `f8878f7` / 603-605 的旧口径；矩阵现为 605/605、未收口 0）。round-98 已记为「建议验收方在下一轮开始前刷新」，非本轮登记范围，故本轮**未重生成**。
4. **`ui-v109.cjs`（13/21）仍未登记进 `run-regression.ps1`**，A-207/A-208 保持 `部分`；**`ui-v48.cjs`（7/14）/ `ui-v51.cjs`（11/19）** 仍是留在仓库但不进门禁的失效脚本（round-99 定的既定状态）。以上均为**既往结论**，round-101 未触及，本轮据实复核后**未改动**。
5. **一处已自愈的文字误差（登记备查，无需动作）**：`819adab` 提交时 backlog 已写「回归锁 18 项」（含 ⑧ `$WaitSeconds`），而该断言对应的代码 `runner-safety.test.cjs` 实为下一提交 `94402e4` 才落地——即 `819adab` 那一刻文件里仍是 16 项。现态已一致（门禁实测 18 项），无残留问题。
...
## round-112（2026-09-21）—— 系统选项按真机「系统设置」对齐

- **已做**：DIFF-67 结清（真机「系统设置」**无**形状下拉，不是改名问题）；新登记 **DIFF-71**；
  `OptionsDialog.tsx` 标题→`系统设置`、页签→`常规`/`打印和数据库`、常规页补四个分组框 + 真机字段原文 + 加速键、
  `启动时运行模板向导` 移到常规页、底部 `保存`→`确定`；`ui-v103` 加严 9/9，`ui-v74/v85/v68/v60/v116` 全绿。
  真机四页签全拍：`parity/reference/labelshop/PROBE-round112-sysset.md`。
- **新增待办（DIFF-71 剩余）**：
  1. 复刻版自造「标签」页（默认标签尺寸/排列/行列间隔/外观形状）真机无对应 → 需定存废（删页 or 记「等价替代」）；
  2. 真机 `打印和数据库`/`编辑`/`系统` 三页字段已取证但无行为实现（表格操作三条、禁用鼠标拖动复制、
     恢复默认窗体布局、恢复模板文档双击链接、授权许可）→ 要先定行为语义再实现，**不许造空壳**；
  3. 复刻版系统选项底部缺真机的 `帮助` 按钮（真机三个：确定/取消/帮助）。
- **DIFF-70 更省的新路径（下一轮建议先试）**：真机「打印」对话框底部有 `预览(V)` 按钮，
  直接看预览即可判「输出里有没有孔」，比再走 Print-to-PDF（round-110 产出空白 A4）便宜得多。

## round-113（实施者）：条码属性「条码」页字段原文对齐 + 颜色定案

- [x] **DIFF-72 已修**：`ObjectPropsDialog.tsx` 条码页字段名逐字对齐真机原文（`条码符号类型(码制)(&B):` / `X 尺寸(&X):` / `码  高(&H):` 两空格用 NBSP+空格实现、单位毫米移到框后 / `位置` / `垂直偏移(&O):` / `对齐方式(&A):`）；
  「条码颜色」从条码页**迁到常规页**并改名 `颜色(&C):`（真机常规页原文）。证据 `parity/reference/labelshop/PROBE-round113-barcode-page.md`（来源＝**已入库**的 `probe-45-barcode-props-p3.txt`，本轮未新增真机驱动）。
  断言：`ui-v125.cjs` 19/19（新增"条码页无颜色控件"+"常规页有 颜色(&C):"两条，原 116/119/120/121 加严为逐字原文）、`ui-v77.cjs` 7/7。
- [ ] **仍未取证**：真机条码页第 8 个**无标签 Edit**（`字符模板` 只是猜测）→ 不实现，保持待取证；取证时用 `Read-LabelShopDialogTree` 打开条码属性停条码页，看该 Edit 的值/UIA 名称。
- [ ] **仍未做**：队列第 1 项 **DIFF-70**（真机打印输出里有没有孔）。本轮评估后**未碰**：`probe-r110-print-dlg.png` 显示点打印时弹出的仍是 **登录 LabelShop** 对话框，"打印链路不可达"的风险高；下一轮建议先按 round-112 的提示试「打印」对话框底部的 **`预览(V)`** 按钮（不需要产出 PDF），若仍弹登录框就直接把结论写成"受限：需登录"并结案。

## round-140（A1/A3 两条打印小改 —— DIFF-91 收口）

- [x] **A1 打印对话框「打印机 名称/位置」占位词** —— 改前传给对话框的是 `打印机` / `Windows 打印机驱动端口`。
  现 `App.tsx` 的 `printPrinterInfo` 取真实数据（模板已选系统打印机 → 该机真实名与端口；未指定 → 系统默认打印机；
  LabelShop 内置打印机仍沿用配置端口），数据源是主进程 `printers:list` 新增的 `port`（`Win32_Printer.PortName`）与 `isDefault`。
- [x] **A3 打印机属性窗口标题写死** —— `打印设置` → `` `<设备名> 属性` ``（真机 probe-15 口径）。
- [x] 新增 `app/scripts/ui-v142.cjs`（6/6），已注册进 `run-regression.ps1`；`Modal.tsx` 增加 `data-testid="modal-title"` 便于精确断言。
- [ ] **A2/A4 复核（非缺口）**：验收方 round-220 已确认 A2（选择标签格式的打印机列表）与 A4（点打印真送出）都不是缺口，本轮未动。
- [ ] 下一轮候选：TOP #2 文本页对齐（三个单选 + `水平对齐(&A):` + `行宽度(&W):` + `字符模板(&T):` 复选+只读输入），
      依据 `parity/P1-对象属性对照表.md` 与 dump `probe-r201-textprops-text-tree.txt`；
      注意 `ui-v78.cjs:89` / `ui-v125.cjs:163` 的 `字体宽度缩放倍数` 断言会在字体页那轮变红（预期内迁移，见 DIFF-90）。
