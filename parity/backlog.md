...
- [ ] A-138、A-151：对齐栏两个分组条目与其下 25 个具体按钮（A-139～A-150、A-152～A-163）仍是 `部分`，只做了存在性/禁用态盘点，缺「按钮=菜单」逐项点击行为断言。来源：`toolbar_align.html`。建议下一轮按同一模式补 `ui-v96.cjs`（对齐六项/旋转三项/尺寸三项/相对标签居中两项/间距两项/顺序四项/位置四项）。
...
- [ ] A-122/A2：格式栏（字体/字号/粗体/斜体/下划线/反白/颜色/文字停靠）与 A-123～A-… 对齐栏 26 个按钮仍只有存在性与禁用态盘点，缺「按钮=菜单」逐项点击行为断言。来源：`toolbar_format.html`、`toolbar_align.html`。
