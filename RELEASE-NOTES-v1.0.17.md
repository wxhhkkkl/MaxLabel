...
- **DIFF-62 第 5 条序列号「边界值」结案：原版没有这个控件**（round-60 真机取证）。
  取证不依赖 UI：控件标题在 Win32 资源里必然是字符串，于是把 `LabelShop.exe` 按 UTF-16LE 与 GBK 全文解码检索——
  「边界值」**0 命中**，而「边界」的 10 处命中全是别的语境（定位裁切标记的「边界距离」、告警语「标签超出了纸张的右侧/底部边界」）；
  同一次检索能命中间一面板的「序列号 / 步长 / 归位 / 显示数据 / 初始值来源」等标题，方法灵敏。帮助也未记载。
  真机序列号面板的资源标题集：类型(&T): / 序列(&Q): / 步长(&S): / 增量(&A) / 减量(&D) / 重复(&E): /
  序列号设置 / 重置 / 提示(&P): / 归位 / 初始值来源(&R): / 数据库连接(&D): / 重置初始值: / 立即重置。
- **新登记 DIFF-63（未收口，待真机点按）**：资源里另有「重置初始值:」与「立即重置」两处字样，帮助未记载，
  真机序列号子面板又是选择时才动态创建（工装切源后不重排，`[ ]` 隐藏控件里也没有序列号面板），
  语义未定 —— **不按猜测实现**。复刻版现状：`序列起始值` 覆盖取值语义，`resetEachRecord` 覆盖归位，无独立「立即重置」入口。

**本轮新增真机工装**：`tools/parity/Probe-LabelShopSerialPage.ps1`（先把数据源下拉切回「常量」再切到目标项以制造真实变更 +
补发 CBN_SELCHANGE/CBN_SELENDOK + 回车，然后 dump 数据源页全部子控件）；证据文件
`parity/reference/labelshop/PROBE-round60.md`、`probe-60-barcode-props-tree.txt`、`probe-60-serial-page.txt`。

其中 `probe-60-barcode-props-tree.txt` 还顺带实证了 DIFF-62 第 3 条的真机形态：
`变量共享名称(&N):` 是 `ComboBox id=1507` 内含 `Edit id=1001`（可下拉 + 可手输），与本轮实现一致。
...
| **DIFF-62** | 五条全部有结论：第 1/2 条（序列只读显示、归位口径）于 v1.0.16；第 3/4 条（共享名下拉、截短整数/小数部分）于本轮；第 5 条「边界值」结案为**原版无此控件**（本轮真机字符串资源取证） |
| **DIFF-63** | **新登记（未收口，待真机点按）**：序列号面板资源里的「重置初始值: / 立即重置」语义未定，不按猜测实现 |
| DIFF-60 | 观察项（真机 EAN/UPC 无「附加条码」、25 码族无独立校验字符，复刻版按原版帮助保留），非缺陷 |
| DIFF-50 | 观察项（真机「水平/垂直」相对标签边对齐下拉是灰的），已按对象类型决定可用性收口一半，真机灰态成因未定 |
...
1. **DIFF-63** 序列号面板「重置初始值 / 立即重置」——需要真机用真实鼠标点选数据源下拉到「序列号」后读子面板并点按一次
   （本机鼠标注入对原版无效，切源消息又不触发页面重排，见 `PROBE-round60.md` 2.4）。
