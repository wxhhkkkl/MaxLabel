# 本轮指定：**DIFF-27 颜色可变打印 → 清剩余待核 → 部分升级**

**当前真实基线（验收方实测，2026-09-16 10:30）**：矩阵 605 条 → 已实现 **376** / 部分 **146** / 未实现 **3** / 待核 **80**（覆盖 **86%**）；A 80% / B 82% / **C 100%** / **D 100%** / E 81%；**未收口差异仅 1 条 = DIFF-27**；门禁全绿（含 UI v52–v91）。**注意别再按旧基线选活**（队列里更早的 DIFF-24/25/26 都已在 round-51 收口）。

**优先级 1 · DIFF-27 对象可变颜色**（唯一未收口差异，要求见 `parity/diffs.md` 的 DIFF-27 全文）：
① `ColorChangeConfig.mode` 扩展为 `fixed | random | indexByContent | indexVar | valueVar | index | rgb`；② 索引表默认注入 **10 个预定义颜色（索引 0–9）**，保留模板公共/对象私有两类；③ 颜色值解析同时支持 `,` 与 `|`；④ 按对象类型限制变色粒度（直线/矩形/图片仅整体；文字整体/逐字符；条码整体/区块/渐变）；⑤ 图片可变颜色仅对单色黑白图启用并给出提示；⑥ 补 CDP 断言（六种模式可选、默认 10 色、两种分隔写法、粒度限制）。必须同时作用于预览与指令输出（预览/位图/指令共享同一 ResolvedPrintScene）。

**优先级 2 · 待核 80**：A 54（`getstart_*` 入门指引系列、`label_page_label`、`label_main_page` 等）+ B 26（数据页/脚本页/条码特性/表单页小簇）。每轮清 8-12 条。

**优先级 3 · 部分 146 升级**：主体是 A 的工具栏/格式栏/对齐栏（81 条存在性已验，缺"点击行为断言"）与各属性页，逐簇补断言即可升级为「已实现」。

**优先级 4 · 未实现 3**：E-11「启动自动更新」可实现（帮助 `install_upgrade.html`）；E-09 硬件锁 / E-10 演示模式为已记录边界，只需在矩阵写明边界理由。

**收口口径**：`已实现` 需两类证据（测试脚本/断言名 + 截图名或命令）；`部分` 必须写清差异；每轮结束前 `powershell -File tools/parity/Check-Matrix.ps1` 必须 exit 0。
...
## A 阶段作战地图（B 收口后执行，验收方按出处聚类）

| 簇 | 出处 | 待核 | 要点 |
| --- | --- | --- | --- |
| A1 主工具栏逐按钮 | `toolbar_mainbar.html` | 39 | 新建/打开/保存、剪切/复制/粘贴/删除、撤销/重做、标签格式设置/预览/打印、对象工具、数据库 7 键、显示组（放大/缩小/适应宽度/适应高度/撑满窗口/帮助主题）——每个按钮的可用性、图标语义与点击效果逐条核对并补断言 |
| A2 对齐栏逐按钮 | `toolbar_align.html` | 26 | 对齐 6 项、旋转 3 项、尺寸 3 项、相对标签居中 2 项、间距 2 项、顺序 4 项、位置 4 项——与 `排列(A)` 菜单同一套回调，需断言"按钮=菜单"行为一致 |
| A3 格式栏逐控件 | `toolbar_format.html` | 16 | 字体/字号、粗体/斜体/下划线/反白、颜色、文字停靠四项、组合/取消组合、属性 |
| A4 系统选项各页 | `config_general.html` | 18 | 常规页各项（界面语言/标尺单位/输出非打印对象/不选中非打印对象/允许运行脚本/启动时运行模板向导/自动旋转输出页面/新建对象后自动打开属性页/工作区背景色）、语言/单位/非打印对象/其它页 |
| A5 右键上下文菜单 | `menu_context.html` | 12 | 画布/对象/标签页三处的菜单项与禁用规则 |
| A6 数据库菜单 | `menu_database.html` | 10 | 10 项的可用性与连库前后差异（含 Ctrl+F 定位记录） |
| A7 其余（选项菜单 7、标签配置 5、颜色 4、首次打印 4、版本 3、界面 3 等） | 各自出处 | ~40 | 逐条核对，能补断言就补 |

**收口口径**：每轮 8-12 条；每条在 `parity/matrix.md` 写证据（测试脚本/断言名 + 截图名）；每轮结束前 `Check-Matrix.ps1` 必须 exit 0。
**本轮必须修 DIFF-24**：① 未连库时数据库工具栏 7 键应禁用；② 未选中对象时 `组合`/`取消组合` 应禁用。两处都按「文档状态 + 选中对象数」的统一可用性来源计算，并补 CDP 断言（未连库 7 键禁用 / 未选中时组合禁用 / 选中两个对象后组合可用）。