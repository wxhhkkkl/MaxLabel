# 本轮指定：**A1/A2/A3 收尾 + DIFF-24 两处禁用规则**

**当前进度**：覆盖 **46%**（已实现 234 / 部分 47 / 未实现 6 / 待核 318）；A 30% / B 35% / C 85% / D 67% / E 81%。**未收口差异 0 条**。
**本轮指定**：按文末「A 阶段作战地图」推进 **A1（`toolbar_mainbar.html` 39 条）与 A2（`toolbar_align.html` 26 条）**——主工具栏每个按钮的可用性/图标语义/点击效果，对齐栏 26 个按钮与 `排列(A)` 菜单同回调的一致性；每轮 10-14 条，写证据（断言名 + 截图）。
**同时**：把 `parity/ACCEPTANCE.md` 的 B 表口径当成验收标准（`已实现` 至少两项证据；`部分` 必须写等价替代）。
**收口口径**：每轮结束前跑 `powershell -File tools/parity/Check-Matrix.ps1`（必须 exit 0）。
