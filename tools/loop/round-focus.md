...
1. **属性页签的顺序与命名不一致（已取证到可动手的程度，按下面改）**
   - **真机文字属性**页签视觉顺序（截图 `probe-63-06-serial-page.png`，验收方逐字读过）：**数据源 / 字体 / 文本 / 常规**（从左到右）。
   - **真机条码属性**的页面集合（控件树 `probe-60-barcode-props-tree.txt`、`probe-48-serial-tree.txt`）：**数据源 / 常规 / 字体 / 条码**
     （另有码制专页；**专页名与视觉顺序尚未取证**——现有素材里没有带页签条的条码属性截图，需要补一张，见下）。
   - 复刻版现状 `app/src/renderer/src/features/object-properties/propertyTabs.ts`：文字 = `通用 / 文字 / 字体 / 数据`；
     条码 = `通用 / 条码 / 字体 / 数据 / <码制>`。**差异**：① 命名：真机 `常规`（复刻 `通用`）、`文本`（复刻 `文字`）、`数据源`（复刻 `数据`）；
     ② 顺序：真机把 **数据源放首位、常规放末位**，复刻版把 `通用` 放首位。
   - 要求：① 先补取证——抓一张（或每类一张）带页签条的真机属性截图，确认条码/图片/RFID/图形/表格各类型的**页签名与顺序**
     （图片/RFID/图形/表格这几类现有素材里也没有页签条截图）；② 按真机改 `propertyTabsFor` 的显示名与顺序
     （key 可以保留英文，但**显示名必须逐字一致**）；③ **补「页签文字与顺序」的 CDP 断言**（现在只有存在性断言）；
     ④ 同步 matrix 里涉及页签的条目证据，并删掉 `propertyTabs.ts` 顶部那句已被实拍证伪的「通用页永远置于首位」注释。
