# 本轮指定：**DIFF-12 收口（唯一未收口差异）+ 矩阵余量**

**当前进度**：覆盖 **46%**（已实现 231 / 部分 47 / 未实现 6 / 待核 321）；A 30% / B 35% / C 82% / D 67% / E 81%。
**未收口差异只剩 1 条**：**DIFF-12 选择标签格式对话框**（7 个子项 + 275 条标签库）——详见 `parity/diffs.md`。
**本轮优先做 DIFF-12**（尤其 12.7 标签库，实现路径已写在 DIFF-12 末尾：从 `sources/LabelFormat360.fmt` 或规格 275 行生成 `labelFormats.generated.ts`，三级联动改读该数据，名称不 Trim/不归一化，补 4 条断言），再做矩阵余量：**A 191 条**（`A 章节收尾` 第 5 项清单已列 10 条具体项）、**B 89 条**（8 簇地图）、**C 18 条**、**D 23 条**。
