...
1. **属性页签的顺序与命名不一致 —— 已登记 `DIFF-64`，验收方 round-5 已把 4 类对象的页签条拍全，按表改即可**
   ...
   ② **把条码的码制专页并入「条码」页**（真机只有 4 个页签，码制专属选项在「条码」页内；改之前先点开真机「条码」页确认字段位置，
   证据截图 `verifier-10-barcode-props.png`）；③ 补**「页签文字与顺序逐项相等」**的 CDP 断言（现在只有存在性断言）；
   ④ 同步 matrix 里涉及页签的 B 章节证据；⑤ 删掉 `propertyTabs.ts` 顶部那句已被实拍证伪的「通用页永远置于首位」注释。
   仍未取证的两类：**图片**属性页（真机未拍）与 **RFID**（见第 3 条）。
...
3. **RFID 工具入口待核对（已登记 `DIFF-65`）**：真机「工具」菜单实测为
   ...
   复刻版工具栏/菜单有 RFID 入口。要求：取证原版 RFID 对象的创建入口（候选 `数据(D)`、其它菜单、特定机型/模板），
   按结果对齐（保留必须有原版入口证据；原版确无则移除或标注等价替代）。证据 `verifier-tools-menu.png`。
