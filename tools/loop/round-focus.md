# 本轮指定：**A-121 工具栏「添加或删除按钮」自定义 + 收平剩余「部分」**

**当前真实基线（验收方实测，2026-09-17 00:30）**：矩阵 605 条 → 已实现 **586** / 部分 **17** / 未实现 **2** / 待核 **0**（覆盖 **100%**）；A/B/C/D 四模块待核全为 0；**未收口差异 0 条**；门禁全绿（round-88 全量 test:ui **58/58**）。**别再按更早的基线选活**。

**优先级 1 · A-121**：帮助 `toolbar_mainbar.html`「**添加或删除按钮** —— 用于添加或删除工具栏按钮，也可自定义按键及布局」。复刻版尚无此功能。要求：① 工具栏提供「添加或删除按钮」入口（下拉或对话框）；② 可勾选显示/隐藏各按钮组（文件/编辑/对象工具/数据库/显示等）；③ 布局与显隐持久化（随系统选项或模板保存）；④ 补 CDP 断言（隐藏某按钮后工具栏不含该按钮、重启后仍生效）。

**优先级 2 · 剩余「部分」收平**（共 17 条，多为"商业版本/授权/服务端内容"，处理原则）：
- **A-202 / A-204 / A-209 / A-210 / A-211 / B-69 / D-36 / D-64**：属**版本分层/授权/指令集文档**类。复刻版是**单版本产品**（见 `app/docs/labelshop-compatibility-audit.md`）：请逐条在矩阵证据列写清「等价替代 / 已记录边界 + 理由」，能补断言的就补（如打印机分类与指令集参数保存、兼容矩阵）。
- **A-271**：起始页右区（广告位/最新文章/下载块）为服务端下发内容 → 在矩阵写明「等价替代：本地不接运营后台，保留同构占位与布局」。
- **E-01/E-02/E-06/E-07/E-08/E-13/E-16**：安装/激活/卸载/技术支持文档性条目 → 逐条对照复刻版实际实现（NSIS 安装包、许可对话框、卸载器、帮助入口）写明对应关系与差异。
- **E-09/E-10（未实现）**：硬件锁激活、专业版演示模式 → 写明「已记录边界：需实体加密狗/版本分层，单版本产品不适用」，并保留在矩阵。

**收口口径**：每轮 6-10 条；`已实现` 需两类证据；`部分` 必须写清差异与等价替代；每轮结束前 `powershell -File tools/parity/Check-Matrix.ps1` 必须 exit 0。
