# 本轮指定：**收口 DIFF-24 / DIFF-25 / DIFF-26**（都是验收方本轮新发现的具体缺口）

**当前进度**：覆盖 **64%**（已实现 242 / 部分 145 / 未实现 8 / 待核 210）；A 61% / B 42% / C 85% / D 67% / E 81%。
**本轮必须做完这三条**（细节见 `parity/diffs.md` 对应小节）：
1. **DIFF-24 工具栏禁用规则**：未连库时数据库工具栏 7 键应禁用；未选中对象时 `组合`/`取消组合` 应禁用；补 3 条 CDP 断言。
2. **DIFF-25 颜色索引表形态**：补表格化编辑（`颜色索引`/`颜色`/`RGB颜色值`/`十六进制` 四列 + 增删行），或在矩阵与台账明确等价替代并支持颜色名与 `#RRGGBB` 两种写法。
3. **DIFF-26 自动旋转输出页面**：系统选项补该开关并持久化；接进打印链路（按纸张方向自动旋转，且必须同时作用于预览与指令输出——遵守"预览/位图/指令共享同一 ResolvedPrintScene"）；补断言（开关存在 + 开启后打印计划的旋转变换不同）。

**之后**：继续按 A/B 两张作战地图清待核（A 110 / B 80 / C 15 / D 23）。
---
---
---
---
